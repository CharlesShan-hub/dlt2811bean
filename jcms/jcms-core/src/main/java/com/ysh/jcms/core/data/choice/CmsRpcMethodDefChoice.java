...
public class CmsRpcMethodDefChoice extends CmsChoice {
    ...
    @Choice(index = 1, name = "method", sync = Sync.WRAPPER)
    ...
    public CmsRpcMethodDefChoice altError(int v) {
        choice(ERROR);
        this.altError.value(v);
        return this;
    }
    public CmsRpcMethodDefChoice altMethod(CmsRpcMethodDef v) {
        choice(METHOD);
        this.altMethod.value(v);
        return this;
    }
    ...
    public CmsRpcMethodDefChoice value(CmsRpcMethodDefChoice v) {
        switch (v.choice()) {
            ...
            case METHOD :
                return altMethod(v.altMethod);
            default :
                throw new IllegalArgumentException("Unknown RpcMethodDefChoice choice: " + v.choice());
        }
    }
}
