...
public enum FunctionalConstraint {
    ...
    ST("Status information", "状态信息",
       ...
       "DataAttribute 表示状态信息，其值可读、替代、报告和日志，但不可写。",
    ...
    MX("Measurands (analogue values)", "测量值（模拟量）",
       ...
       "DataAttribute 表示测量值信息，其值可读、替代、报告和日志，但不可写。",
       "Initial value of the DataAttribute shall be taken from the process.",
       "DataAttribute 的初始值应从过程获取。"),
    ...
    OR("Operate received", "操作接收",
       ...
       "DataAttribute 表示接收 Operate 请求的数据对象上的 Operate 请求结果，即使 Operate 的执行被阻塞。",
       "Initial value is irrelevant / arbitrary",
       "初始值无关/任意"),

    BL("Blocking", "阻塞",
       "DataAttribute is used for blocking value updates.",
       "DataAttribute 用于阻塞值更新。",
       "If the value of the DataAttribute is volatile then the initial value shall be FALSE, else the value should be as set or configured.",
       "若 DataAttribute 值为易失性，则初始值应为 FALSE，否则应为已设置或配置的值。"),
    ...
    FunctionalConstraint(String semantic, String semanticZh,
                         String servicesAllowed, String servicesAllowedZh,
                         String initialValue, String initialValueZh) {
    ...
    /** FC 语义说明（中文） */
    public String semanticZh() { return semanticZh; }

    /** 允许的服务/操作（英文） */
    public String servicesAllowed() { return servicesAllowed; }
    /** 允许的服务/操作（中文） */
    public String servicesAllowedZh() { return servicesAllowedZh; }

    /** 初始值 / 存储行为（英文） */
    public String initialValue() { return initialValue; }
    /** 初始值 / 存储行为（中文） */
    public String initialValueZh() { return initialValueZh; }
