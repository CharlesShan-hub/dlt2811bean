...
/**
 * MSVCBValue ::= CHOICE { error [0] IMPLICIT ServiceError, value [1] IMPLICIT
 * MSVCB } — 8.10.2 — used by GetMSVCBValues-ResponsePDU (SEQUENCE OF).
 */
public class CmsMsvcbValueChoice extends CmsChoice {
    ...
    @Choice(index = 1, name = "value", sync = Sync.WRAPPER) public CmsMsvcb altValue;

    public CmsMsvcbValueChoice() {
        super(new InnerAnonymousGetMSVCBValuesResponsePDUMsvcb());
    }

    public CmsMsvcbValueChoice choice(int v) { super.choice(v); return this; }

    /* ─── Fluent setters (set choice + value in one call) ─── */
    public CmsMsvcbValueChoice altError(int v) { choice(ERROR); this.altError.value(v); return this; }
    public CmsMsvcbValueChoice altValue(CmsMsvcb v) { choice(VALUE); this.altValue.value(v); return this; }
