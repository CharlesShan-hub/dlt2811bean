...
import com.ysh.jcms.util.CmsEqualityUtil;
 ...
 */
public abstract class CmsTypeOld {

    private static final Logger log = LoggerFactory.getLogger(CmsTypeOld.class);

    public Pointer nativePtr;
    public int nativeSize;
    protected Codec codec;

    protected CmsTypeOld() {
        this.nativeSize = calcNativeSize();
        this.nativePtr = new Memory(nativeSize);
        zero();
    }

    protected CmsTypeOld(Codec codec) {
        this();
        this.codec = codec;
    }
     ...
     */
    public void write() {
        List<? extends CmsTypeOld> kids = children();
     ...
     */
    protected List<? extends CmsTypeOld> resizeList() {
        return children();
    }

    protected void resize() {
        List<? extends CmsTypeOld> kids = resizeList();
        for (int i = 0; i < kids.size(); i++) {
            CmsTypeOld child = kids.get(i);
            if (child != null)
                child.resize();
    ...
    protected String toString(int depth) {
        ...
        java.util.Map<CmsTypeOld, String> fieldNames = new IdentityHashMap<>();
        for (java.lang.reflect.Field f : getClass().getFields()) {
            if (CmsTypeOld.class.isAssignableFrom(f.getType())) {
                try {
                    fieldNames.put((CmsTypeOld) f.get(this), f.getName());
                } catch (Exception e) {
                }
            }
        }
