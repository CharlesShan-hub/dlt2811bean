...
public class CmsGetAllCbValuesTest {
    ...
    public void response_roundup_with_brcb_entry() {
        ...
        entry1.reference("cbRef1");
        entry1.value.choice(CmsCbValueChoice.BRCB);
        entry1.value.altBrcb.rptID("rpt01");
        entry1.value.altBrcb.rptEna(true);
        entry1.value.altBrcb.confRev(42L);
        entry1.value.altBrcb.optFlds.sequence_number(true);
        entry1.value.altBrcb.sqNum(99);
        entry1.value.altBrcb.trgOps.data_change(true);
        entry1.value.altBrcb.intgPd(5000L);
        entry1.value.altBrcb.gi(false);
        entry1.value.altBrcb.purgeBuf(true);
        entry1.value.altBrcb.entryID(new byte[]{1, 2, 3, 4, 5, 6, 7, 8});

        CmsCbValueEntry entry2 = new CmsCbValueEntry();
        entry2.reference("cbRef2");
        entry2.value.choice(CmsCbValueChoice.GOCB);
        entry2.value.altGocb.goEna(true);
        entry2.value.altGocb.confRev(100L);
        entry2.value.altGocb.ndsCom(false);
    ...
    public void error_roundup() {
        CmsGetAllCbValuesError a = new CmsGetAllCbValuesError().value(com.ysh.jcms.data.common.CmsServiceError.INSTANCE_LOCKED_BY_OTHER_CLIENT);
        byte[] encoded = a.encode();
