...
public class CmsGooseTest {
    ...
    public void get_gocb_request() {
        CmsGetGoCbValuesRequest a = new CmsGetGoCbValuesRequest().reqId(1);
    ...
    public void send_goose_message() {
        CmsSendGooseMessage a = new CmsSendGooseMessage().reqId(20).goId("goID1".getBytes()).datSetPresent(false).goRefPresent(false)
                .stNum(1L).sqNum(100L).simulation(false).confRev(5L).ndsCom(false);
    ...
    public void simple_error() {
        CmsGetGoCbValuesError a = new CmsGetGoCbValuesError().reqId(99).serviceError(CmsServiceError.INSTANCE_NOT_AVAILABLE);
        byte[] encoded = a.encode();
