...
public final class DataWriterResolver {
     ...
     *
     * @param nav  已导航到目标点的 Navigator（需包含 LN）
     * @param value 字符串值
     * @return CmsServiceError.NO_ERROR，或错误码
     */
    public static int setValue(Navigator nav, String value) {
        if (!nav.isValid() || nav.ln() == null) return CmsServiceError.INSTANCE_NOT_AVAILABLE;
        if (nav.ref().isLnLevel()) return CmsServiceError.INSTANCE_NOT_AVAILABLE;
        ...
        for (String sdiName : nav.ref().sdiChain()) {
            ...
            if (next == null) {
                ...
                if (currentSdi == null) doi.addSdi(next);
                else currentSdi.addSdi(next);
            }
            currentSdi = next;
        }
        ...
        SclDAI dai = (currentSdi != null)
                ? currentSdi.findDaiByName(nav.ref().daName())
                : doi.findDaiByName(nav.ref().daName());
        if (dai == null) {
            dai = new SclDAI().name(nav.ref().daName());
            if (currentSdi != null) currentSdi.addDai(dai);
            else doi.addDai(dai);
        }
    ...
    private static String findFirstDaName(Navigator nav) {
        ...
        if (step.doType() == null || step.doType().das().isEmpty()) return null;
