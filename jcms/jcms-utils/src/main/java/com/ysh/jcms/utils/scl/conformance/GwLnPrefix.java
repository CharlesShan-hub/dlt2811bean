...
public enum GwLnPrefix {
    ...
    CB("Circuit breaker (XCBR/CSWI/CILO)"),
    QG("Disconnector (XSWI)"),
    QGD("Earthing switch"),

    // I.2 functional abbreviations (prefix examples)
    Lin("Line measurement (LinMMXU1 / LinMMXN1)"),
    Bus("Bus measurement (BusMMXU1 / BusMMXN1)"),
    CBSyn("Breaker synchronising control (CBSynCSWI1)"),
    CBDea("Breaker dead-line control (CBDeaCSWI1)"),
    BinIn("Binary input status (BinInGGIOx)"),
    DevAlm("Device alarm (DevAlmGGIOx)"),
    GOAlm("GOOSE alarm (GOAlmGGIOx)"),
    PP("Phase-phase distance (PPPDIS2)"),
    PG("Phase-ground distance (PGPDIS3)"),
    Ph("Phase overcurrent (PhPTOC2)"),
    Zer("Zero-sequence overcurrent (ZerPTOC3)"),
    CarDis("Carrier distance (CarDisPDIS / CarDisPSCH)"),
    CarZer("Carrier zero-sequence (CarZerPTOC / CarZerPSCH)"),
    PctDif("Percentage differential (PctDifPDIF)"),
    HiSet("High-set differential (HiSetPDIF)"),
    RemTr("Remote trip (RemTrPSCH)"),
    Cha("Charging protection (ChaPTOC)"),
    PTFail("PT failure overcurrent (PTFailPTOC)");
