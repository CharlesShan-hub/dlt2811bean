...
public class SetDataValuesServer extends BaseServerHandler {
    ...
    protected Frame onDecodeSuccess(Session session, CmsType rawReq) {
        ...
        if (successCount < req.data.count) {
            ...
            try { return buildError(err.encode(), reqId); }
            catch (Exception ex) { return onDecodeError(reqId, CmsServiceError.FAILED_DUE_TO_SERVER_CONSTRAINT); }
        }
    ...
    private static String extractValue(CmsData data) {
        ...
        switch (ct) {
            case CmsData.CHOICE_BOOLEAN:       return Boolean.toString(data.alt_boolean.value());
            case CmsData.CHOICE_INT8:           return Integer.toString(data.alt_int8.value());
            case CmsData.CHOICE_INT16:          return Integer.toString(data.alt_int16.value());
            case CmsData.CHOICE_INT32:          return Integer.toString(data.alt_int32.value());
            case CmsData.CHOICE_INT64:          return Long.toString(data.alt_int64.value());
            case CmsData.CHOICE_INT8U:          return Integer.toString(data.alt_int8u.value());
            case CmsData.CHOICE_INT16U:         return Integer.toString(data.alt_int16u.value());
            case CmsData.CHOICE_INT32U:         return Long.toString(data.alt_int32u.value());
            case CmsData.CHOICE_INT64U:         return data.alt_int64u.value().toString();
            case CmsData.CHOICE_FLOAT32:        return Float.toString(data.alt_float32.value());
            case CmsData.CHOICE_FLOAT64:        return Double.toString(data.alt_float64.value());
            case CmsData.CHOICE_VISIBLE_STRING: return str(data.alt_visible_string.value());
            case CmsData.CHOICE_UNICODE_STRING: return str(data.alt_unicode_string.value());
            case CmsData.CHOICE_OCTET_STRING:   return str(data.alt_octet_string.value());
            default:                            return null;
        }
    }
}
