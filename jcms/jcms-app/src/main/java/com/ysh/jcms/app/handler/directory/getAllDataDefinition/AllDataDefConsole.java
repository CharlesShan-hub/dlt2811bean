...
public class AllDataDefConsole implements CommandHandler {
    ...
    @Override
    public String description() { return "获取所有数据定义 (GetAllDataDefinition)。用法: all-def --ln <ldName|lnReference> [--fc FC] [--after REF] [--json]"; }
        ...
        if (jsonMode) {
            ...
            for (int i = 0; i < entries.size(); i++) {
                ...
                sb.append("{\"ref\":\"").append(CmsFormatUtil.escapeJson(e.reference))
                  ...
                  .append("\",\"cdc\":\"").append(CmsFormatUtil.escapeJson(e.cdcType)).append("\"}");
            }
            sb.append("]}");
