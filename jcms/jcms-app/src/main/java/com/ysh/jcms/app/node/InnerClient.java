...
public class InnerClient implements ConnectionListener {
    ...
    public void reportHandler(Consumer<Frame> handler) {
    ...
    public void close() {
        // State hook: entering DISCONNECTED clears pending requests and closes the
        // connection
        if (session != null) {
    ...
    public void onFrameReceived(Connection connection, Frame frame) {
        try {
            ...
            if (frame.header().resp()) {
                log.warn("Unmatched response: service={}, reqId={}, asduLen={}, pending={}", frame.header().serviceCode(), frame.reqId(),
                        frame.asduBytes() != null ? frame.asduBytes().length : 0, session.pendingCount());
            }
        } catch (Exception e) {
