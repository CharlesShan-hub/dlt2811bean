...
public class GetLcbValuesConsole implements CommandHandler {
    ...
    @Override
    public String name() { return "get-lcb-vals"; }

    @Override
    public String description() { return "获取日志控制块值 (GetLCBValues)。用法: get-lcb-vals --refs \"<ref1> <ref2>...\" [--json]"; }
    ...
    public void execute(CmsConsole console, Map<String, String> args) throws Exception {
        ...
        if (refsStr == null || refsStr.trim().isEmpty()) {
            if (jsonMode) {
                ...
                ConsolePrinter.error("Missing --refs. Usage: get-lcb-vals --refs \"<ref1> <ref2>...\"");
            }
            return;
        }
        ...
        List<GetLcbValuesClient.LcbEntry> entries =
            console.getClient(GetLcbValuesClient.class).getLastEntries();

        if (entries.isEmpty()) {
            if (jsonMode) {
                ConsolePrinter.raw("{\"success\":true,\"data\":[]}");
            } else {
                ConsolePrinter.info("No LCB values returned");
            }
            return;
        }
        ...
        if (jsonMode) {
            ...
            for (int i = 0; i < entries.size(); i++) {
                ...
                sb.append("{\"ref\":\"").append(CmsFormatUtil.escapeJson(ref))
                  .append("\",\"desc\":\"").append(CmsFormatUtil.escapeJson(entries.get(i).desc)).append("\"}");
            }
            sb.append("]}");
