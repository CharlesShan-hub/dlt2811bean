 ...
 * {
 *     &#64;code
 *     List<CmsDataValueEntry> entries = CursorSlicer.after(allEntries, refAfter, e -> e.reference.value(), reqId);
 * }
 * </pre>
 */
public final class CursorSlicer {
     ...
     *            the element type
     * @param items
     *            the full list (must be sorted by reference)
     * @param cursor
     *            the cursor reference (may be null or empty, in which case the full
     *            list is returned)
     * @param refExtractor
     *            extracts the reference string from each element
     * @param reqId
     *            request ID for error reporting
     * @return the sublist after the cursor
     * @throws ServiceException
     *             with {@code INSTANCE_NOT_AVAILABLE} if the cursor is not found in
     *             the list
     */
