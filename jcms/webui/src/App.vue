...
const navItems = [
  {
    ...
    label: '连接管理',
    icon: '🔌',
    children: CMD_IDS.filter((id) => id !== 'server-dir').map((id) => ({ id, label: CMD_DEFS[id].title })),
  },
  { id: 'dir-tree', label: '目录树', icon: '⊞', children: [{ id: 'server-dir', label: CMD_DEFS['server-dir'].title }] },
