...
public class CmsURCB extends AbstractCmsCompound<CmsURCB> {
    ...
    public CmsURCB decode(byte[] data) {
        ffiDecode(data);
        ns.read();
        syncFromNative();
        return this;
    ...
    protected int ffiEncode(byte[] buf, IntByReference outLen) {
        return CmsFFIDatatypes.INSTANCE.cms_urcb_encode(ns, buf, outLen);
    }

    @Override
    protected void ffiDecode(byte[] data) {
        CmsFFIDatatypes.INSTANCE.cms_urcb_decode(data, data.length, ns);
        ns.read();
        syncFromNative();
    }
