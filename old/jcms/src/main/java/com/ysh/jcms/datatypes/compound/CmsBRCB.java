...
public class CmsBRCB extends AbstractCmsCompound<CmsBRCB> {
    ...
    @Override
    protected List<String> getFieldOrder() {
        return Arrays.asList(); // struct fields handled via NativeStruct
    }
    ...
    public CmsBRCB decode(byte[] data) {
        ffiDecode(data);
        ns.read();
        syncFromNative();
        return this;
    ...
    protected int ffiEncode(byte[] buf, IntByReference outLen) {
        return CmsFFIDatatypes.INSTANCE.cms_brcb_encode(ns, buf, outLen);
    }

    @Override
    protected void ffiDecode(byte[] data) {
        CmsFFIDatatypes.INSTANCE.cms_brcb_decode(data, data.length, ns);
        ns.read();
        syncFromNative();
    }
