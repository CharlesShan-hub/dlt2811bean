#ifndef CMS_SCALAR_INT64_H
#define CMS_SCALAR_INT64_H

#include "cms_types.h"
#include "per/cms_stream.h"

#ifdef __cplusplus
extern "C" {
#endif

/* Int64 ::= INTEGER (-2^63..2^63-1)  —  7.1.2 */
typedef struct { int64_t value; } cms_int64_t;

int cms_int64_encode_stream(per_stream_t *s, const void *ptr);
int cms_int64_decode_stream(per_stream_t *s, void *ptr);

CMS_EXPORT int cms_int64_encode(const void *ptr, uint8_t **out_buf, size_t *out_len);
CMS_EXPORT int cms_int64_decode(void *ptr, const uint8_t *in_buf, int in_len);

#ifdef __cplusplus
}
#endif

#endif
