...
int cms_get_brcb_values_request_encode(const cms_get_brcb_values_request_t *pdu, uint8_t **out_buf, size_t *out_len) {
    per_stream_t s;
...
int cms_get_brcb_values_response_encode(const cms_get_brcb_values_response_t *pdu, uint8_t **out_buf, size_t *out_len) {
    per_stream_t s;
    per_error_t err_init = per_stream_init_write(&s, 64);
    if (err_init) return (int)err_init;
    int err;
    ...
    {
        uint32_t cnt = (uint32_t)pdu->brcb->count;
        per_error_t perr = per_encode_length(&s, cnt);
        if (perr) { per_stream_free(&s); return CMS_ERR; }
        for (uint32_t i = 0; i < cnt; i++) {
            cms_rcb_value_choice_t *e = (cms_rcb_value_choice_t*)pdu->brcb->elements[i];
            if (!e) { per_stream_free(&s); return CMS_ERR; }
            err = cms_rcb_value_choice_encode_stream(&s, e);
            if (err) { per_stream_free(&s); return err; }
        }
    }
...
int cms_get_brcb_values_response_decode(cms_get_brcb_values_response_t *pdu, const uint8_t *in_buf, int in_len) {
    ...
    if (!pdu->brcb) return CMS_ERR;
    {
        ...
        int inner_retry_needed = 0;
        for (uint32_t i = 0; i < cnt; i++) {
            err = cms_rcb_value_choice_decode_stream(&s, retry_needed ? NULL : pdu->brcb->elements[i]);
            if (err == CMS_RETRY) inner_retry_needed = 1;
            else if (err) return err;
        }
        if (inner_retry_needed) retry_needed = 1;
...
int cms_get_brcb_values_error_encode(const cms_get_brcb_values_error_t *pdu, uint8_t **out_buf, size_t *out_len) {
    per_stream_t s;
    per_error_t err_init = per_stream_init_write(&s, 64);
    if (err_init) return (int)err_init;
    int err;
