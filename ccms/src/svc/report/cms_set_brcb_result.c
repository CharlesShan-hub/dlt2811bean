...
int cms_set_brcb_result_encode_stream(per_stream_t *s, const cms_set_brcb_result_t *v) {
    ...
    /* 0. OPTIONAL bitmap (12 fields: error, rptID, rptEna, datSet, optFlds, bufTm, trgOps, intgPd, gi, purgeBuf,
     * entryID, resvTms) */
    bool opt_present[12] = {(v->error_present && v->error_present->value) && v->error,
                            ...
                            (v->purge_buf_err_present && v->purge_buf_err_present->value) && v->purge_buf_err,
                            (v->entry_id_err_present && v->entry_id_err_present->value) && v->entry_id_err,
                            (v->resv_tms_err_present && v->resv_tms_err_present->value) && v->resv_tms_err};
    err = (int) per_encode_optional_bitmap(s, opt_present, 12);
    if (err)
        return err;
    ...
    /* 10. purgeBufErr — ServiceError OPTIONAL (bitmap[9]) */
    if (opt_present[9]) {
        err = cms_service_error_encode_stream(s, v->purge_buf_err);
        if (err)
            return err;
    }

    /* 11. entryIdErr — ServiceError OPTIONAL (bitmap[10]) */
    if (opt_present[10]) {
        err = cms_service_error_encode_stream(s, v->entry_id_err);
        if (err)
            return err;
    }

    /* 12. resvTmsErr — ServiceError OPTIONAL (bitmap[11]) */
    if (opt_present[11]) {
        err = cms_service_error_encode_stream(s, v->resv_tms_err);
        if (err)
            return err;
    }
...
int cms_set_brcb_result_decode_stream(per_stream_t *s, void *ptr) {
    ...
    if (v) {
        ...
        if (v->purge_buf_err_present)
            v->purge_buf_err_present->value = opt_present[9];
        if (v->entry_id_err_present)
            v->entry_id_err_present->value = opt_present[10];
        if (v->resv_tms_err_present)
            v->resv_tms_err_present->value = opt_present[11];
    }
    ...
    /* 10. purgeBufErr OPTIONAL */ if (opt_present[9]) {
        if (v && !v->purge_buf_err)
            return CMS_ERR;
        err = cms_service_error_decode_stream(s, v ? v->purge_buf_err : NULL);
        if (err)
            return err;
    }
    /* 11. entryIdErr OPTIONAL */ if (opt_present[10]) {
        if (v && !v->entry_id_err)
            return CMS_ERR;
        err = cms_service_error_decode_stream(s, v ? v->entry_id_err : NULL);
        if (err)
            return err;
    }
    /* 12. resvTmsErr OPTIONAL */ if (opt_present[11]) {
        if (v && !v->resv_tms_err)
            return CMS_ERR;
        err = cms_service_error_decode_stream(s, v ? v->resv_tms_err : NULL);
        if (err)
            return err;
    }
