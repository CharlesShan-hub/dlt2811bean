...
int cms_set_urcb_result_encode_stream(per_stream_t *s, const cms_set_urcb_result_t *v) {
    ...
    /* 10. resvErr — ServiceError OPTIONAL */
    { int p=(v->resv_err_present&&v->resv_err_present->value)&&v->resv_err; cms_boolean_t b={.value=p}; err=cms_boolean_encode_stream(s,&b); if(err)return err; if(p){err=cms_service_error_encode_stream(s,v->resv_err);if(err)return err;} }
...
int cms_set_urcb_result_decode_stream(per_stream_t *s, cms_set_urcb_result_t *v) {
    ...
    /* 10. resvErr OPTIONAL */
    { cms_boolean_t b={0}; err=cms_boolean_decode_stream(s,&b); if(err)return err; if(v->resv_err_present)v->resv_err_present->value=b.value; if(b.value&&v->resv_err){err=cms_service_error_decode_stream(s,v->resv_err);if(err)return err;} }
