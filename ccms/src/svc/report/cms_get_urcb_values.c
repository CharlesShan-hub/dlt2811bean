#include "svc/report/cms_get_urcb_values.h"
...
int cms_get_urcb_values_request_encode(const cms_get_urcb_values_request_t *pdu, uint8_t **out_buf, size_t *out_len) {
    per_stream_t s;
    ...
    if (!pdu->reference) { per_stream_free(&s); return CMS_ERR; } { uint32_t cnt=(uint32_t)pdu->reference->count; per_error_t perr=per_encode_length(&s,cnt); if(perr){per_stream_free(&s);return CMS_ERR;} for(uint32_t i=0;i<cnt;i++){cms_object_reference_t*e=(cms_object_reference_t*)pdu->reference->elements[i];if(!e){per_stream_free(&s);return CMS_ERR;}err=cms_object_reference_encode_stream(&s,e);if(err){per_stream_free(&s);return err;} } }
...
int cms_get_urcb_values_response_encode(const cms_get_urcb_values_response_t *pdu, uint8_t **out_buf, size_t *out_len) {
    ...
    if (!pdu->urcb) { per_stream_free(&s); return CMS_ERR; }
    {
        ...
        for (uint32_t i = 0; i < cnt; i++) {
            cms_rcb_value_choice_t *e = (cms_rcb_value_choice_t*)pdu->urcb->elements[i];
