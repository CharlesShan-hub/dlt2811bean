...
int cms_get_urcb_values_request_encode(const cms_get_urcb_values_request_t *pdu, uint8_t *out_buf, int *out_len) {
...
int cms_get_urcb_values_response_encode(const cms_get_urcb_values_response_t *pdu, uint8_t *out_buf, int *out_len) {
    per_stream_t s; per_stream_init_write(&s, out_buf, (size_t)*out_len); int err;
    if (!pdu->req_id) return CMS_ERR; err = cms_req_id_encode_stream(&s, pdu->req_id); if (err) return err;
    if (!pdu->urcb) return CMS_ERR; { uint32_t cnt=(uint32_t)pdu->urcb->count; per_error_t perr=per_encode_length(&s,cnt); if(perr)return CMS_ERR; for(uint32_t i=0;i<cnt;i++){cms_rcb_value_choice_t*e=(cms_rcb_value_choice_t*)pdu->urcb->elements[i];if(!e)return CMS_ERR;err=cms_rcb_value_choice_encode_stream(&s,e);if(err)return err;} }
...
int cms_get_urcb_values_error_encode(const cms_get_urcb_values_error_t *pdu, uint8_t *out_buf, int *out_len) {
    per_stream_t s; per_stream_init_write(&s, out_buf, (size_t)*out_len); int err;
    if (!pdu->req_id) return CMS_ERR; err = cms_req_id_encode_stream(&s, pdu->req_id); if (err) return err;
