...
int cms_report_encode(const cms_report_t *pdu, uint8_t *out_buf, int *out_len) {
    ...
    bool opt_present[6] = {
        (pdu->sq_num_present && pdu->sq_num_present->value) && pdu->sq_num,
...
int cms_report_decode(cms_report_t *pdu, const uint8_t *in_buf, int in_len) {
    ...
    if (err) return err;
    if (pdu->sq_num_present) pdu->sq_num_present->value = opt_present[0];
    if (pdu->sub_seq_num_present) pdu->sub_seq_num_present->value = opt_present[1];
    if (pdu->more_segments_follow_present) pdu->more_segments_follow_present->value = opt_present[2];
    if (pdu->data_set_present) pdu->data_set_present->value = opt_present[3];
    if (pdu->buf_ovfl_present) pdu->buf_ovfl_present->value = opt_present[4];
    if (pdu->conf_rev_present) pdu->conf_rev_present->value = opt_present[5];
