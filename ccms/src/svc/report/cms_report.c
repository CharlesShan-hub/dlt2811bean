...
int cms_report_encode(const cms_report_t *pdu, uint8_t *out_buf, int *out_len) {
    ...
    /* 5. subSeqNum — INT16U OPTIONAL */
    {
        int present = (pdu->sub_seq_num_present && pdu->sub_seq_num_present->value) && pdu->sub_seq_num;
        cms_boolean_t bit = { .value = present };
        err = cms_boolean_encode_stream(&s, &bit);
        if (err) return err;
        if (present) {
    ...
    /* 6. moreSegmentsFollow — BOOLEAN OPTIONAL */
    {
        int present = (pdu->more_segments_follow_present && pdu->more_segments_follow_present->value);
        cms_boolean_t bit = { .value = present };
        err = cms_boolean_encode_stream(&s, &bit);
        if (err) return err;
        if (present) {
    ...
    /* 7. dataSet — ObjectReference OPTIONAL */
    {
        int present = (pdu->data_set_present && pdu->data_set_present->value) && pdu->data_set;
        cms_boolean_t bit = { .value = present };
        err = cms_boolean_encode_stream(&s, &bit);
        if (err) return err;
        if (present) {
    ...
    /* 8. bufOvfl — BOOLEAN OPTIONAL */
    {
        int present = (pdu->buf_ovfl_present && pdu->buf_ovfl_present->value);
        cms_boolean_t bit = { .value = present };
        err = cms_boolean_encode_stream(&s, &bit);
        if (err) return err;
        if (present) {
    ...
    /* 9. confRev — INT32U OPTIONAL */
    {
        int present = (pdu->conf_rev_present && pdu->conf_rev_present->value) && pdu->conf_rev;
        cms_boolean_t bit = { .value = present };
        err = cms_boolean_encode_stream(&s, &bit);
        if (err) return err;
        if (present) {
