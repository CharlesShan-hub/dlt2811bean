...
int cms_set_urcb_entry_encode_stream(per_stream_t *s, const cms_set_urcb_entry_t *v) {
    ...
    { int p = (v->resv_present && v->resv_present->value) && v->resv; cms_boolean_t b={.value=p}; err=cms_boolean_encode_stream(&s,&b); if(err)return err; if(p){err=cms_boolean_encode_stream(s,v->resv);if(err)return err;} }
    return CMS_OK;
}
int cms_set_urcb_entry_decode_stream(per_stream_t *s, cms_set_urcb_entry_t *v) {
    ...
    { cms_boolean_t b={0}; err=cms_boolean_decode_stream(&s,&b); if(err)return err; if(v->resv_present)v->resv_present->value=b.value; if(b.value&&v->resv){err=cms_boolean_decode_stream(s,v->resv);if(err)return err;} }
    return CMS_OK;
}
