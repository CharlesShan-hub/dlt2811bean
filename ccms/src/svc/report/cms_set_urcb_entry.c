...
int cms_set_urcb_entry_encode_stream(per_stream_t *s, const cms_set_urcb_entry_t *v) {
    ...
    bool opt_present[9] = {(v->rpt_id_present && v->rpt_id_present->value) && v->rpt_id,
...
int cms_set_urcb_entry_decode_stream(per_stream_t *s, void *ptr) {
    ...
    if (v) {
        ...
        if (v->resv_present)
            v->resv_present->value = opt_present[8];
    }
    ...
    /* 10. resv OPTIONAL */ if (opt_present[8]) {
        if (v && !v->resv)
            return CMS_ERR;
        err = cms_boolean_decode_stream(s, v ? v->resv : NULL);
        if (err)
            return err;
    }
