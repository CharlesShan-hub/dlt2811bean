...
int cms_set_brcb_values_request_encode(const cms_set_brcb_values_request_t *pdu, uint8_t **out_buf, size_t *out_len) {
    per_stream_t s;
    ...
    err = cms_req_id_encode_stream(&s, pdu->req_id); if (err) { per_stream_free(&s); return err; }

    if (!pdu->brcb) { per_stream_free(&s); return CMS_ERR; }
    { uint32_t cnt=(uint32_t)pdu->brcb->count; per_error_t perr=per_encode_length(&s,cnt); if(perr){per_stream_free(&s);return CMS_ERR;} for(uint32_t i=0;i<cnt;i++){cms_set_brcb_entry_t*e=(cms_set_brcb_entry_t*)pdu->brcb->elements[i];if(!e){per_stream_free(&s);return CMS_ERR;}err=cms_set_brcb_entry_encode_stream(&s,e);if(err){per_stream_free(&s);return err;} } }
...
/* ── Error ── */
int cms_set_brcb_values_error_encode(const cms_set_brcb_values_error_t *pdu, uint8_t **out_buf, size_t *out_len) {
    per_stream_t s;
    per_error_t err_init = per_stream_init_write(&s, 64);
    if (err_init) return (int)err_init;
    int err;
    ...
    { uint32_t cnt=(uint32_t)pdu->result->count; per_error_t perr=per_encode_length(&s,cnt); if(perr){per_stream_free(&s);return CMS_ERR;} for(uint32_t i=0;i<cnt;i++){cms_set_brcb_result_t*e=(cms_set_brcb_result_t*)pdu->result->elements[i];if(!e){per_stream_free(&s);return CMS_ERR;}err=cms_set_brcb_result_encode_stream(&s,e);if(err){per_stream_free(&s);return err;} } }
    *out_buf = per_stream_detach(&s, out_len); return CMS_OK;
}
