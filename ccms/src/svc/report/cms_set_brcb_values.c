...
int cms_set_brcb_values_request_encode(const cms_set_brcb_values_request_t *pdu, uint8_t **out_buf, size_t *out_len) {
    ...
    }
    {
        ...
        for (uint32_t i = 0; i < cnt; i++) {
            ...
            err = cms_set_brcb_entry_encode_stream(&s, e);
            if (err) {
                per_stream_free(&s);
                return err;
            }
...
int cms_set_brcb_values_error_encode(const cms_set_brcb_values_error_t *pdu, uint8_t **out_buf, size_t *out_len) {
    ...
    }
    {
        ...
        for (uint32_t i = 0; i < cnt; i++) {
            cms_set_brcb_result_t *e = (cms_set_brcb_result_t *) pdu->result->elements[i];
            if (!e) {
                per_stream_free(&s);
                return CMS_ERR;
            }
            err = cms_set_brcb_result_encode_stream(&s, e);
            if (err) {
                per_stream_free(&s);
                return err;
            }
...
int cms_set_brcb_values_error_decode(cms_set_brcb_values_error_t *pdu, const uint8_t *in_buf, int in_len) {
        ...
        return CMS_ERR;
    {
        ...
        for (uint32_t i = 0; i < cnt; i++) {
            ...
            if (!e)
                return CMS_ERR;
            err = cms_set_brcb_result_decode_stream(&s, e);
            if (err)
                return err;
        }
    }
