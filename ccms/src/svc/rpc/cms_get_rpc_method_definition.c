...
int cms_get_rpc_method_definition_request_encode(const cms_get_rpc_method_definition_request_t *pdu, uint8_t **out_buf,
                                                 size_t *out_len) {
    per_stream_t s;
...
int cms_get_rpc_method_definition_request_decode(cms_get_rpc_method_definition_request_t *pdu, const uint8_t *in_buf,
                                                 int in_len) {
    per_stream_t s;
...
int cms_get_rpc_method_definition_response_encode(const cms_get_rpc_method_definition_response_t *pdu,
...
int cms_get_rpc_method_definition_response_decode(cms_get_rpc_method_definition_response_t *pdu, const uint8_t *in_buf,
                                                  int in_len) {
    per_stream_t s;
    per_stream_init_read(&s, in_buf, (size_t) in_len);
    int err;
        ...
        return CMS_ERR;
    {
        ...
        int inner_retry_needed = 0;
        for (uint32_t i = 0; i < cnt; i++) {
            err = cms_rpc_method_def_choice_decode_stream(&s, retry_needed ? NULL : pdu->reference->elements[i]);
            if (err == CMS_RETRY)
                inner_retry_needed = 1;
            else if (err)
                return err;
...
int cms_get_rpc_method_definition_error_encode(const cms_get_rpc_method_definition_error_t *pdu, uint8_t **out_buf,
                                               size_t *out_len) {
    per_stream_t s;
...
int cms_get_rpc_method_definition_error_decode(cms_get_rpc_method_definition_error_t *pdu, const uint8_t *in_buf,
                                               int in_len) {
    per_stream_t s;
    per_stream_init_read(&s, in_buf, (size_t) in_len);
    int err;
