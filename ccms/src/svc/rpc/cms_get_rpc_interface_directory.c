#include "svc/rpc/cms_get_rpc_interface_directory.h"
...
int cms_get_rpc_interface_directory_request_encode(const cms_get_rpc_interface_directory_request_t*pdu,uint8_t*out_buf,int*out_len){
    ...
    if(!pdu->ref_after_present&&!pdu->ref_after)return CMS_ERR;{int p=(pdu->ref_after_present&&pdu->ref_after_present->value)&&pdu->ref_after;cms_boolean_t b={.value=p};err=cms_boolean_encode_stream(&s,&b);if(err)return err;if(p){err=cms_visible_string_encode_stream(&s,pdu->ref_after,UINT32_MAX);if(err)return err;}}
    *out_len=(int)per_stream_bytes_written(&s);return CMS_OK;
}
int cms_get_rpc_interface_directory_request_decode(cms_get_rpc_interface_directory_request_t*pdu,const uint8_t*in_buf,int in_len){
...
int cms_get_rpc_interface_directory_response_encode(const cms_get_rpc_interface_directory_response_t*pdu,uint8_t*out_buf,int*out_len){
    per_stream_t s;per_stream_init_write(&s,out_buf,(size_t)*out_len);int err;
    if(!pdu->req_id)return CMS_ERR;err=cms_req_id_encode_stream(&s,pdu->req_id);if(err)return err;
...
int cms_get_rpc_interface_directory_response_decode(cms_get_rpc_interface_directory_response_t*pdu,const uint8_t*in_buf,int in_len){
    per_stream_t s;per_stream_init_read(&s,in_buf,(size_t)in_len);int err;
    if(!pdu->req_id)return CMS_ERR;err=cms_req_id_decode_stream(&s,pdu->req_id);if(err)return err;
...
int cms_get_rpc_interface_directory_error_encode(const cms_get_rpc_interface_directory_error_t*pdu,uint8_t*out_buf,int*out_len){
    per_stream_t s;per_stream_init_write(&s,out_buf,(size_t)*out_len);int err;
    if(!pdu->req_id)return CMS_ERR;err=cms_req_id_encode_stream(&s,pdu->req_id);if(err)return err;
    if(!pdu->service_error)return CMS_ERR;err=cms_service_error_encode_stream(&s,pdu->service_error);if(err)return err;
    *out_len=(int)per_stream_bytes_written(&s);return CMS_OK;
}
int cms_get_rpc_interface_directory_error_decode(cms_get_rpc_interface_directory_error_t*pdu,const uint8_t*in_buf,int in_len){
    per_stream_t s;per_stream_init_read(&s,in_buf,(size_t)in_len);int err;
    if(!pdu->req_id)return CMS_ERR;err=cms_req_id_decode_stream(&s,pdu->req_id);if(err)return err;
