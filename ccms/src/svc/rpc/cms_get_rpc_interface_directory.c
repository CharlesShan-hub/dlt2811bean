...
int cms_get_rpc_interface_directory_request_encode(const cms_get_rpc_interface_directory_request_t *pdu, uint8_t **out_buf, size_t *out_len) {
    per_stream_t s;
    ...
    /* 2. refAfter — VisibleString OPTIONAL (bitmap[0]) */
    if (opt_present[0]) {
...
int cms_get_rpc_interface_directory_response_encode(const cms_get_rpc_interface_directory_response_t *pdu, uint8_t **out_buf, size_t *out_len) {
    per_stream_t s;
    ...
    /* 1. reference — SEQUENCE OF VisibleString */
...
int cms_get_rpc_interface_directory_error_encode(const cms_get_rpc_interface_directory_error_t *pdu, uint8_t **out_buf, size_t *out_len) {
    per_stream_t s;
    per_error_t err_i = per_stream_init_write(&s, 64);
    if (err_i) return (int)err_i;
    int err;
