...
int cms_get_rpc_method_directory_request_encode(const cms_get_rpc_method_directory_request_t*pdu,uint8_t*out_buf,int*out_len){
    ...
    {int p=(pdu->interface_present&&pdu->interface_present->value)&&pdu->interface_name;cms_boolean_t b={.value=p};err=cms_boolean_encode_stream(&s,&b);if(err)return err;if(p){err=cms_visible_string_encode_stream(&s,pdu->interface_name,UINT32_MAX);if(err)return err;}}
