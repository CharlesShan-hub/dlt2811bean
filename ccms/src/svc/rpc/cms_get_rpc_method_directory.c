...
int cms_get_rpc_method_directory_request_encode(const cms_get_rpc_method_directory_request_t *pdu, uint8_t **out_buf, size_t *out_len) {
    per_stream_t s;
    ...
    bool opt_present[2] = {
        (pdu->interface_present && pdu->interface_present->value) && pdu->interface_name,
...
int cms_get_rpc_method_directory_request_decode(cms_get_rpc_method_directory_request_t *pdu, const uint8_t *in_buf, int in_len) {
    ...
    if (err) return err;
    if (pdu->interface_present) pdu->interface_present->value = opt_present[0];
    if (pdu->ref_after_present) pdu->ref_after_present->value = opt_present[1];
...
int cms_get_rpc_method_directory_response_encode(const cms_get_rpc_method_directory_response_t *pdu, uint8_t **out_buf, size_t *out_len) {
    per_stream_t s;
...
int cms_get_rpc_method_directory_error_encode(const cms_get_rpc_method_directory_error_t *pdu, uint8_t **out_buf, size_t *out_len) {
    per_stream_t s;
    per_error_t err_i = per_stream_init_write(&s, 64);
    if (err_i) return (int)err_i;
    int err;
