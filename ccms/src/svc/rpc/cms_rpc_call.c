...
int cms_rpc_call_request_encode(const cms_rpc_call_request_t*pdu,uint8_t*out_buf,int*out_len){
    ...
    if(!pdu->method)return CMS_ERR;err=cms_visible_string_encode_stream(&s,pdu->method,UINT32_MAX);if(err)return err;
    if(!pdu->req)return CMS_ERR;err=cms_rpc_call_req_choice_encode_stream(&s,pdu->req);if(err)return err;
...
int cms_rpc_call_request_decode(cms_rpc_call_request_t*pdu,const uint8_t*in_buf,int in_len){
    ...
    if(!pdu->method)return CMS_ERR;err=cms_visible_string_decode_stream(&s,pdu->method,UINT32_MAX);if(err)return err;
    if(!pdu->req)return CMS_ERR;err=cms_rpc_call_req_choice_decode_stream(&s,pdu->req);if(err)return err;
    return CMS_OK;
}
int cms_rpc_call_response_encode(const cms_rpc_call_response_t*pdu,uint8_t*out_buf,int*out_len){
    per_stream_t s;per_stream_init_write(&s,out_buf,(size_t)*out_len);int err;
    if(!pdu->req_id)return CMS_ERR;err=cms_req_id_encode_stream(&s,pdu->req_id);if(err)return err;
    if(!pdu->rsp_data)return CMS_ERR;err=cms_data_encode_stream(&s,pdu->rsp_data);if(err)return err;
    {int p=(pdu->next_call_id_present&&pdu->next_call_id_present->value)&&pdu->next_call_id;cms_boolean_t b={.value=p};err=cms_boolean_encode_stream(&s,&b);if(err)return err;if(p){err=cms_octet_string_encode_stream(&s,pdu->next_call_id,UINT32_MAX);if(err)return err;}}
    *out_len=(int)per_stream_bytes_written(&s);return CMS_OK;
}
int cms_rpc_call_response_decode(cms_rpc_call_response_t*pdu,const uint8_t*in_buf,int in_len){
    per_stream_t s;per_stream_init_read(&s,in_buf,(size_t)in_len);int err;
    if(!pdu->req_id)return CMS_ERR;err=cms_req_id_decode_stream(&s,pdu->req_id);if(err)return err;
    if(!pdu->rsp_data)return CMS_ERR;err=cms_data_decode_stream(&s,pdu->rsp_data);if(err)return err;
    {cms_boolean_t b={0};err=cms_boolean_decode_stream(&s,&b);if(err)return err;if(pdu->next_call_id_present)pdu->next_call_id_present->value=b.value;if(b.value&&pdu->next_call_id){err=cms_octet_string_decode_stream(&s,pdu->next_call_id,UINT32_MAX);if(err)return err;}}
    return CMS_OK;
}
