...
int cms_rpc_call_response_encode(const cms_rpc_call_response_t *pdu, uint8_t **out_buf, size_t *out_len) {
    ...
    bool opt_present[1] = {(pdu->next_call_id_present && pdu->next_call_id_present->value) && pdu->next_call_id};
...
int cms_rpc_call_response_decode(cms_rpc_call_response_t *pdu, const uint8_t *in_buf, int in_len) {
    ...
    if (err)
        return err;
    if (pdu->next_call_id_present)
        pdu->next_call_id_present->value = opt_present[0];
