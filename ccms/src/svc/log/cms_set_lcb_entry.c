...
int cms_set_lcb_entry_encode_stream(per_stream_t *s, const cms_set_lcb_entry_t *v) {
    ...
    bool opt_present[7] = {(v->log_ena_present && v->log_ena_present->value) && v->log_ena,
...
int cms_set_lcb_entry_decode_stream(per_stream_t *s, void *ptr) {
    ...
    if (err)
        return err;
    if (v) {
        if (v->log_ena_present)
            v->log_ena_present->value = opt_present[0];
        if (v->dat_set_present)
            v->dat_set_present->value = opt_present[1];
        if (v->trg_ops_present)
            v->trg_ops_present->value = opt_present[2];
        if (v->intg_pd_present)
            v->intg_pd_present->value = opt_present[3];
        if (v->log_ref_present)
            v->log_ref_present->value = opt_present[4];
        if (v->opt_flds_present)
            v->opt_flds_present->value = opt_present[5];
        if (v->buf_tm_present)
            v->buf_tm_present->value = opt_present[6];
    }

#define DECODE_OPT(field, fn, arg)                                                                                     \
    do {                                                                                                               \
        if (opt_present[field]) {                                                                                      \
            if (v && !(arg))                                                                                           \
                return CMS_ERR;                                                                                        \
            err = fn(s, v ? (arg) : NULL);                                                                             \
            if (err)                                                                                                   \
