...
int cms_log_status_value_choice_encode_stream(per_stream_t *s, const cms_log_status_value_choice_t *v) {
    ...
    switch (sel) {
    case CMS_LOG_STATUS_VALUE_CHOICE_ERROR:
        if (!v->alt_error) return CMS_ERR;
        return cms_service_error_encode_stream(s, v->alt_error);
    case CMS_LOG_STATUS_VALUE_CHOICE_VALUE:
        if (!v->alt_value) return CMS_ERR;
        return cms_log_status_value_encode_stream(s, v->alt_value);
    default:
        return CMS_ERR;
    }
}

int cms_log_status_value_choice_decode_stream(per_stream_t *s, void *ptr) {
    cms_log_status_value_choice_t *v = (cms_log_status_value_choice_t*)ptr;
    uint32_t sel;
    ...
    switch (sel) {
    case CMS_LOG_STATUS_VALUE_CHOICE_ERROR:
        if (v && !v->alt_error) return CMS_ERR;
        return cms_service_error_decode_stream(s, v ? v->alt_error : NULL);
    case CMS_LOG_STATUS_VALUE_CHOICE_VALUE:
        if (v && !v->alt_value) return CMS_ERR;
        return cms_log_status_value_decode_stream(s, v ? v->alt_value : NULL);
    default:
        return CMS_ERR;
    }
}
