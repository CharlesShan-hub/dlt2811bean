...
int cms_query_log_by_time_request_encode(const cms_query_log_by_time_request_t *pdu, uint8_t **out_buf,
                                         size_t *out_len) {
    per_stream_t s;
    ...
    bool opt_present[3] = {(pdu->start_time_present && pdu->start_time_present->value) && pdu->start_time,
...
int cms_query_log_by_time_request_decode(cms_query_log_by_time_request_t *pdu, const uint8_t *in_buf, int in_len) {
    ...
    if (err)
        return err;
    if (pdu->start_time_present)
        pdu->start_time_present->value = opt_present[0];
    if (pdu->stop_time_present)
        pdu->stop_time_present->value = opt_present[1];
    if (pdu->entry_after_present)
        pdu->entry_after_present->value = opt_present[2];
...
int cms_query_log_by_time_response_encode(const cms_query_log_by_time_response_t *pdu, uint8_t **out_buf,
                                          size_t *out_len) {
    per_stream_t s;
...
int cms_query_log_by_time_error_encode(const cms_query_log_by_time_error_t *pdu, uint8_t **out_buf, size_t *out_len) {
    per_stream_t s;
    per_error_t err_init = per_stream_init_write(&s, 64);
    if (err_init)
        return (int) err_init;
