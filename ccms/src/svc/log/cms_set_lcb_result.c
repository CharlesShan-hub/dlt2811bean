...
int cms_set_lcb_result_decode_stream(per_stream_t *s, void *ptr) {
    cms_set_lcb_result_t *v = (cms_set_lcb_result_t*)ptr;
    int err;
    ...
    /* 2. logEnaErr OPTIONAL */ if (opt_present[1]) { if (v && !v->log_ena_err) return CMS_ERR; err = cms_service_error_decode_stream(s, v ? v->log_ena_err : NULL); if (err) return err; }
    /* 3. datSetErr OPTIONAL */ if (opt_present[2]) { if (v && !v->dat_set_err) return CMS_ERR; err = cms_service_error_decode_stream(s, v ? v->dat_set_err : NULL); if (err) return err; }
    /* 4. trgOpsErr OPTIONAL */ if (opt_present[3]) { if (v && !v->trg_ops_err) return CMS_ERR; err = cms_service_error_decode_stream(s, v ? v->trg_ops_err : NULL); if (err) return err; }
    /* 5. intgPdErr OPTIONAL */ if (opt_present[4]) { if (v && !v->intg_pd_err) return CMS_ERR; err = cms_service_error_decode_stream(s, v ? v->intg_pd_err : NULL); if (err) return err; }
    /* 6. logRefErr OPTIONAL */ if (opt_present[5]) { if (v && !v->log_ref_err) return CMS_ERR; err = cms_service_error_decode_stream(s, v ? v->log_ref_err : NULL); if (err) return err; }
    ...
    /* 8. bufTmErr OPTIONAL */  if (opt_present[7]) { if (v && !v->buf_tm_err) return CMS_ERR; err = cms_service_error_decode_stream(s, v ? v->buf_tm_err : NULL); if (err) return err; }
