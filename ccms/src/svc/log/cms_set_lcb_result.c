...
int cms_set_lcb_result_encode_stream(per_stream_t *s, const cms_set_lcb_result_t *v) {
     ...
     */
    bool opt_present[8] = {(v->error_present && v->error_present->value) && v->error,
...
int cms_set_lcb_result_decode_stream(per_stream_t *s, void *ptr) {
    ...
    if (v) {
        ...
        if (v->log_ena_err_present)
            v->log_ena_err_present->value = opt_present[1];
        if (v->dat_set_err_present)
            v->dat_set_err_present->value = opt_present[2];
        if (v->trg_ops_err_present)
            v->trg_ops_err_present->value = opt_present[3];
        if (v->intg_pd_err_present)
            v->intg_pd_err_present->value = opt_present[4];
        if (v->log_ref_err_present)
            v->log_ref_err_present->value = opt_present[5];
        if (v->opt_flds_err_present)
            v->opt_flds_err_present->value = opt_present[6];
        if (v->buf_tm_err_present)
            v->buf_tm_err_present->value = opt_present[7];
    }
    ...
    /* 2. logEnaErr OPTIONAL */ if (opt_present[1]) {
        if (v && !v->log_ena_err)
            return CMS_ERR;
        err = cms_service_error_decode_stream(s, v ? v->log_ena_err : NULL);
        if (err)
            return err;
    }
    /* 3. datSetErr OPTIONAL */ if (opt_present[2]) {
        if (v && !v->dat_set_err)
            return CMS_ERR;
    ...
    /* 4. trgOpsErr OPTIONAL */ if (opt_present[3]) {
        if (v && !v->trg_ops_err)
            return CMS_ERR;
    ...
    /* 5. intgPdErr OPTIONAL */ if (opt_present[4]) {
        if (v && !v->intg_pd_err)
            return CMS_ERR;
    ...
    /* 6. logRefErr OPTIONAL */ if (opt_present[5]) {
        if (v && !v->log_ref_err)
            return CMS_ERR;
        err = cms_service_error_decode_stream(s, v ? v->log_ref_err : NULL);
        if (err)
            return err;
    }
    /* 7. optFldsErr OPTIONAL */ if (opt_present[6]) {
        if (v && !v->opt_flds_err)
            return CMS_ERR;
    ...
    /* 8. bufTmErr OPTIONAL */ if (opt_present[7]) {
        if (v && !v->buf_tm_err)
            return CMS_ERR;
