...
int cms_log_data_entry_encode_stream(per_stream_t *s, const cms_log_data_entry_t *v) {
    if (!v) {
        fprintf(stderr, "[CMS_DEBUG] log_data_entry_encode: v is NULL\n");
        return CMS_ERR;
    }
    if (!v->reference) {
        fprintf(stderr, "[CMS_DEBUG] log_data_entry_encode: reference is NULL\n");
        return CMS_ERR;
    }
    if (!v->fc) {
        fprintf(stderr, "[CMS_DEBUG] log_data_entry_encode: fc is NULL\n");
        return CMS_ERR;
    }
    if (!v->value) {
        fprintf(stderr, "[CMS_DEBUG] log_data_entry_encode: value is NULL\n");
        return CMS_ERR;
    }
    if (!v->reason) {
        fprintf(stderr, "[CMS_DEBUG] log_data_entry_encode: reason is NULL\n");
        return CMS_ERR;
    }
    int err;
    ...
    if (err) {
        fprintf(stderr, "[CMS_DEBUG] log_data_entry_encode: reference failed err=%d\n", err);
        return err;
    }
    ...
    if (err) {
        fprintf(stderr, "[CMS_DEBUG] log_data_entry_encode: fc failed err=%d\n", err);
        return err;
    }
    ...
    if (err) {
        fprintf(stderr, "[CMS_DEBUG] log_data_entry_encode: value failed err=%d\n", err);
        return err;
    }
    ...
    if (err) {
        fprintf(stderr, "[CMS_DEBUG] log_data_entry_encode: reason failed err=%d\n", err);
        return err;
    }
