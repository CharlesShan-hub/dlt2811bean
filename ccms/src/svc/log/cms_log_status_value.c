...
int cms_log_status_value_encode_stream(per_stream_t *s, const cms_log_status_value_t *v) {
    if (!v || !v->old_entr_tm || !v->new_entr_tm || !v->old_entr || !v->new_entr) return CMS_ERR;
    int err;
...
int cms_log_status_value_decode_stream(per_stream_t *s, cms_log_status_value_t *v) {
    ...
    err = cms_entry_time_decode_stream(s, v->old_entr_tm);
    if (err) return err;

    err = cms_entry_time_decode_stream(s, v->new_entr_tm);
    if (err) return err;

    err = cms_entry_id_decode_stream(s, v->old_entr);
    if (err) return err;

    err = cms_entry_id_decode_stream(s, v->new_entr);
    if (err) return err;
