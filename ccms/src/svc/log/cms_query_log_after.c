...
int cms_query_log_after_request_encode(const cms_query_log_after_request_t *pdu,uint8_t *out_buf,int *out_len){
    ...
    if(!pdu->entry)return CMS_ERR;err=cms_entry_id_encode_stream(&s,pdu->entry);if(err)return err;
...
int cms_query_log_after_request_decode(cms_query_log_after_request_t *pdu,const uint8_t *in_buf,int in_len){
    ...
    if(!pdu->entry)return CMS_ERR;err=cms_entry_id_decode_stream(&s,pdu->entry);if(err)return err;
    return CMS_OK;
}
int cms_query_log_after_response_encode(const cms_query_log_after_response_t *pdu,uint8_t *out_buf,int *out_len){
    per_stream_t s;per_stream_init_write(&s,out_buf,(size_t)*out_len);int err;
    if(!pdu->req_id)return CMS_ERR;err=cms_req_id_encode_stream(&s,pdu->req_id);if(err)return err;
...
int cms_query_log_after_error_encode(const cms_query_log_after_error_t *pdu,uint8_t *out_buf,int *out_len){
    per_stream_t s;per_stream_init_write(&s,out_buf,(size_t)*out_len);int err;
    if(!pdu->req_id)return CMS_ERR;err=cms_req_id_encode_stream(&s,pdu->req_id);if(err)return err;
