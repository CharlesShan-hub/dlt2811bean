...
int cms_get_file_directory_request_encode(const cms_get_file_directory_request_t *pdu, uint8_t **out_buf,
                                          size_t *out_len) {
    per_stream_t s;
...
int cms_get_file_directory_request_decode(cms_get_file_directory_request_t *pdu, const uint8_t *in_buf, int in_len) {
    ...
    if (pdu->file_after_present)
        pdu->file_after_present->value = opt_present[2];
...
int cms_get_file_directory_response_encode(const cms_get_file_directory_response_t *pdu, uint8_t **out_buf,
                                           size_t *out_len) {
    per_stream_t s;
...
int cms_get_file_directory_response_decode(cms_get_file_directory_response_t *pdu, const uint8_t *in_buf, int in_len) {
        ...
        return CMS_ERR;
    {
        ...
        for (uint32_t i = 0; i < cnt; i++) {
            err = cms_file_entry_decode_stream(&s, retry_needed ? NULL : pdu->file_entry->elements[i]);
            if (err == CMS_RETRY)
