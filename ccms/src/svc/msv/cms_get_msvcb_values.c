...
int cms_get_msvcb_values_request_encode(const cms_get_msvcb_values_request_t*pdu,uint8_t*out_buf,int*out_len){
...
int cms_get_msvcb_values_response_encode(const cms_get_msvcb_values_response_t*pdu,uint8_t*out_buf,int*out_len){
    per_stream_t s;per_stream_init_write(&s,out_buf,(size_t)*out_len);int err;
    if(!pdu->req_id)return CMS_ERR;err=cms_req_id_encode_stream(&s,pdu->req_id);if(err)return err;
    if(!pdu->msvcb)return CMS_ERR;{uint32_t cnt=(uint32_t)pdu->msvcb->count;per_error_t perr=per_encode_length(&s,cnt);if(perr)return CMS_ERR;for(uint32_t i=0;i<cnt;i++){cms_msvcb_value_choice_t*e=(cms_msvcb_value_choice_t*)pdu->msvcb->elements[i];if(!e)return CMS_ERR;err=cms_msvcb_value_choice_encode_stream(&s,e);if(err)return err;}}
...
int cms_get_msvcb_values_response_decode(cms_get_msvcb_values_response_t*pdu,const uint8_t*in_buf,int in_len){
    per_stream_t s;per_stream_init_read(&s,in_buf,(size_t)in_len);int err;
    if(!pdu->req_id)return CMS_ERR;err=cms_req_id_decode_stream(&s,pdu->req_id);if(err)return err;
    if(!pdu->msvcb)return CMS_ERR;{uint32_t cnt;per_error_t perr=per_decode_length(&s,&cnt);if(perr)return CMS_ERR;pdu->msvcb->count=(int32_t)cnt;for(uint32_t i=0;i<cnt;i++){cms_msvcb_value_choice_t*e=(cms_msvcb_value_choice_t*)pdu->msvcb->elements[i];if(!e)return CMS_ERR;err=cms_msvcb_value_choice_decode_stream(&s,e);if(err)return err;}}
    if(!pdu->more_follows)return CMS_ERR;err=cms_boolean_decode_stream(&s,pdu->more_follows);if(err)return err;
    return CMS_OK;
}
int cms_get_msvcb_values_error_encode(const cms_get_msvcb_values_error_t*pdu,uint8_t*out_buf,int*out_len){
    per_stream_t s;per_stream_init_write(&s,out_buf,(size_t)*out_len);int err;
    if(!pdu->req_id)return CMS_ERR;err=cms_req_id_encode_stream(&s,pdu->req_id);if(err)return err;
