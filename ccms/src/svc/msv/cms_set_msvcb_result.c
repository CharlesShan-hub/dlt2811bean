...
int cms_set_msvcb_result_encode_stream(per_stream_t *s, const cms_set_msvcb_result_t *v) {
    ...
    bool opt_present[7] = {(v->error_present && v->error_present->value) && v->error,
...
int cms_set_msvcb_result_decode_stream(per_stream_t *s, void *ptr) {
    ...
    if (v) {
        ...
        if (v->sv_ena_err_present)
            v->sv_ena_err_present->value = opt_present[1];
        if (v->msv_id_err_present)
            v->msv_id_err_present->value = opt_present[2];
        if (v->dat_set_err_present)
            v->dat_set_err_present->value = opt_present[3];
        if (v->smp_mod_err_present)
            v->smp_mod_err_present->value = opt_present[4];
        if (v->smp_rate_err_present)
            v->smp_rate_err_present->value = opt_present[5];
    ...
    /* 2. svEnaErr OPTIONAL */ if (opt_present[1]) {
        if (v && !v->sv_ena_err)
            return CMS_ERR;
        err = cms_service_error_decode_stream(s, v ? v->sv_ena_err : NULL);
        if (err)
            return err;
    }
    /* 3. msvIdErr OPTIONAL */ if (opt_present[2]) {
        if (v && !v->msv_id_err)
            return CMS_ERR;
        err = cms_service_error_decode_stream(s, v ? v->msv_id_err : NULL);
        if (err)
            return err;
    }
    ...
    /* 5. smpModErr OPTIONAL */ if (opt_present[4]) {
        if (v && !v->smp_mod_err)
            return CMS_ERR;
        err = cms_service_error_decode_stream(s, v ? v->smp_mod_err : NULL);
        if (err)
            return err;
    }
    /* 6. smpRateErr OPTIONAL */ if (opt_present[5]) {
        if (v && !v->smp_rate_err)
            return CMS_ERR;
        err = cms_service_error_decode_stream(s, v ? v->smp_rate_err : NULL);
        if (err)
            return err;
    }
