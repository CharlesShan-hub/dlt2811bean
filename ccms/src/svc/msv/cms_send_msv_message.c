...
int cms_send_msv_message_encode(const cms_send_msv_message_t *pdu, uint8_t **out_buf, size_t *out_len) {
    ...
    bool opt_present[4] = {(pdu->dat_set_present && pdu->dat_set_present->value) && pdu->dat_set,
...
int cms_send_msv_message_decode(cms_send_msv_message_t *pdu, const uint8_t *in_buf, int in_len) {
    ...
    if (err)
        return err;
    if (pdu->dat_set_present)
        pdu->dat_set_present->value = opt_present[0];
    if (pdu->ref_tm_present)
        pdu->ref_tm_present->value = opt_present[1];
    if (pdu->smp_rate_present)
        pdu->smp_rate_present->value = opt_present[2];
    if (pdu->smp_mod_present)
        pdu->smp_mod_present->value = opt_present[3];
        ...
        return CMS_ERR;
    {
        ...
        for (uint32_t i = 0; i < cnt; i++) {
            err = cms_data_decode_stream(&s, retry_needed ? NULL : pdu->sample->elements[i]);
            if (err == CMS_RETRY)
