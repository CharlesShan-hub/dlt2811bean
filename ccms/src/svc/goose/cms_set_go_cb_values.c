...
int cms_set_go_cb_values_request_decode(cms_set_go_cb_values_request_t *pdu, const uint8_t *in_buf, int in_len) {
        ...
        return CMS_ERR;
    {
        ...
        for (uint32_t i = 0; i < cnt; i++) {
            err = cms_set_go_cb_entry_decode_stream(&s, retry_needed ? NULL : pdu->gocb->elements[i]);
            if (err == CMS_RETRY)
...
int cms_set_go_cb_values_response_encode(const cms_set_go_cb_values_response_t *pdu, uint8_t **out_buf,
                                         size_t *out_len) {
    per_stream_t s;
    per_error_t err_init = per_stream_init_write(&s, 64);
    if (err_init)
...
int cms_set_go_cb_values_error_decode(cms_set_go_cb_values_error_t *pdu, const uint8_t *in_buf, int in_len) {
        ...
        return CMS_ERR;
    {
        ...
        int inner_retry_needed = 0;
        for (uint32_t i = 0; i < cnt; i++) {
            err = cms_set_go_cb_result_decode_stream(&s, retry_needed ? NULL : pdu->result->elements[i]);
            if (err == CMS_RETRY)
                inner_retry_needed = 1;
            else if (err)
                return err;
