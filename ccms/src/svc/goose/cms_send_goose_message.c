...
int cms_send_goose_message_encode(const cms_send_goose_message_t *pdu, uint8_t **out_buf, size_t *out_len) {
    ...
    bool opt_present[2] = {(pdu->dat_set_present && pdu->dat_set_present->value) && pdu->dat_set,
...
int cms_send_goose_message_decode(cms_send_goose_message_t *pdu, const uint8_t *in_buf, int in_len) {
    ...
    if (pdu->go_ref_present)
        pdu->go_ref_present->value = opt_present[1];
        ...
        return CMS_ERR;
    {
        ...
        for (uint32_t i = 0; i < cnt; i++) {
            err = cms_data_decode_stream(&s, retry_needed ? NULL : pdu->data->elements[i]);
            if (err == CMS_RETRY)
