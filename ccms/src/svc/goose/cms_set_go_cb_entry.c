...
int cms_set_go_cb_entry_encode_stream(per_stream_t *s, const cms_set_go_cb_entry_t *v) {
    ...
    bool opt_present[3] = {(v->go_ena_present && v->go_ena_present->value) && v->go_ena,
...
int cms_set_go_cb_entry_decode_stream(per_stream_t *s, void *ptr) {
    ...
    if (v) {
        if (v->go_ena_present)
            v->go_ena_present->value = opt_present[0];
        if (v->go_id_present)
            v->go_id_present->value = opt_present[1];
        if (v->dat_set_present)
            v->dat_set_present->value = opt_present[2];
    }

    /* 2. goEna OPTIONAL */ if (opt_present[0]) {
        if (v && !v->go_ena)
            return CMS_ERR;
        err = cms_boolean_decode_stream(s, v ? v->go_ena : NULL);
        if (err)
            return err;
    }
    /* 3. goID OPTIONAL */ if (opt_present[1]) {
        if (v && !v->go_id)
            return CMS_ERR;
        err = cms_visible_string_decode_stream(s, v ? v->go_id : NULL, 129);
        if (err)
            return err;
    }
