...
int cms_get_go_reference_request_encode(const cms_get_go_reference_request_t *pdu, uint8_t **out_buf, size_t *out_len) {
    per_stream_t s;
    ...
    /* 3. memberOfs — SEQUENCE OF INT16U */
    if (!pdu->member_ofs) { per_stream_free(&s); return CMS_ERR; }
    {
        uint32_t cnt = (uint32_t)pdu->member_ofs->count;
...
int cms_get_go_reference_request_decode(cms_get_go_reference_request_t *pdu, const uint8_t *in_buf, int in_len) {
    ...
    /* 3. memberOfs */
    if (!pdu->member_ofs) return CMS_ERR;
    {
        uint32_t cnt;
...
int cms_get_go_reference_response_encode(const cms_get_go_reference_response_t *pdu, uint8_t **out_buf, size_t *out_len) {
    per_stream_t s;
    per_error_t err_init = per_stream_init_write(&s, 64);
    if (err_init) return (int)err_init;
    int err;
    ...
    /* 5. memberData — SEQUENCE OF GoRefFcEntry */
...
int cms_get_go_reference_error_encode(const cms_get_go_reference_error_t *pdu, uint8_t **out_buf, size_t *out_len) {
    per_stream_t s;
    per_error_t err_init = per_stream_init_write(&s, 64);
    if (err_init) return (int)err_init;
    int err;
