...
int cms_get_goose_element_number_request_encode(const cms_get_goose_element_number_request_t*pdu,uint8_t*out_buf,int*out_len){
...
int cms_get_goose_element_number_response_encode(const cms_get_goose_element_number_response_t*pdu,uint8_t*out_buf,int*out_len){
    per_stream_t s;per_stream_init_write(&s,out_buf,(size_t)*out_len);int err;
    if(!pdu->req_id)return CMS_ERR;err=cms_req_id_encode_stream(&s,pdu->req_id);if(err)return err;
    if(!pdu->gocb_reference)return CMS_ERR;err=cms_object_reference_encode_stream(&s,pdu->gocb_reference);if(err)return err;
    ...
    if(!pdu->member_offset)return CMS_ERR;{uint32_t cnt=(uint32_t)pdu->member_offset->count;per_error_t perr=per_encode_length(&s,cnt);if(perr)return CMS_ERR;for(uint32_t i=0;i<cnt;i++){cms_int16u_t*e=(cms_int16u_t*)pdu->member_offset->elements[i];if(!e)return CMS_ERR;err=cms_int16u_encode_stream(&s,e);if(err)return err;}}
    *out_len=(int)per_stream_bytes_written(&s);return CMS_OK;
}
int cms_get_goose_element_number_response_decode(cms_get_goose_element_number_response_t*pdu,const uint8_t*in_buf,int in_len){
    ...
    if(!pdu->member_offset)return CMS_ERR;{uint32_t cnt;per_error_t perr=per_decode_length(&s,&cnt);if(perr)return CMS_ERR;pdu->member_offset->count=(int32_t)cnt;for(uint32_t i=0;i<cnt;i++){cms_int16u_t*e=(cms_int16u_t*)pdu->member_offset->elements[i];if(!e)return CMS_ERR;err=cms_int16u_decode_stream(&s,e);if(err)return err;}}
    return CMS_OK;
}
int cms_get_goose_element_number_error_encode(const cms_get_goose_element_number_error_t*pdu,uint8_t*out_buf,int*out_len){
    per_stream_t s;per_stream_init_write(&s,out_buf,(size_t)*out_len);int err;
    if(!pdu->req_id)return CMS_ERR;err=cms_req_id_encode_stream(&s,pdu->req_id);if(err)return err;
