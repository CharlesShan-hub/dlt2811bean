...
int cms_time_activated_operate_termination_encode(const cms_time_activated_operate_termination_t *pdu,
    ...
    /* 1. OPTIONAL bitmap (1 field: addCause) */
    bool opt[1] = {(pdu->add_cause_present && pdu->add_cause_present->value) && pdu->add_cause};
    ...
    /* 10. addCause — AddCause OPTIONAL (bitmap[0]) */
    if (opt[0]) {
...
int cms_time_activated_operate_termination_decode(cms_time_activated_operate_termination_t *pdu, const uint8_t *in_buf,
                                                  int in_len) {
    per_stream_t s;
    ...
    if (pdu->add_cause_present)
        pdu->add_cause_present->value = opt[0] ? 1 : 0;
