#include "svc/control/cms_command_termination.h"
...
int cms_command_termination_encode(const cms_command_termination_t*pdu,uint8_t*out_buf,int*out_len){
    ...
    {int p=(pdu->oper_tm_present&&pdu->oper_tm_present->value)&&pdu->oper_tm;cms_boolean_t b={.value=p};err=cms_boolean_encode_stream(&s,&b);if(err)return err;if(p){err=cms_time_stamp_encode_stream(&s,pdu->oper_tm);if(err)return err;}}
    ...
    {int p=(pdu->add_cause_present&&pdu->add_cause_present->value)&&pdu->add_cause;cms_boolean_t b={.value=p};err=cms_boolean_encode_stream(&s,&b);if(err)return err;if(p){err=cms_add_cause_encode_stream(&s,pdu->add_cause);if(err)return err;}}
    *out_len=(int)per_stream_bytes_written(&s);return CMS_OK;
}
int cms_command_termination_decode(cms_command_termination_t*pdu,const uint8_t*in_buf,int in_len){
    ...
    {cms_boolean_t b={0};err=cms_boolean_decode_stream(&s,&b);if(err)return err;if(pdu->oper_tm_present)pdu->oper_tm_present->value=b.value;if(b.value&&pdu->oper_tm){err=cms_time_stamp_decode_stream(&s,pdu->oper_tm);if(err)return err;}}
    ...
    {cms_boolean_t b={0};err=cms_boolean_decode_stream(&s,&b);if(err)return err;if(pdu->add_cause_present)pdu->add_cause_present->value=b.value;if(b.value&&pdu->add_cause){err=cms_add_cause_decode_stream(&s,pdu->add_cause);if(err)return err;}}
    return CMS_OK;
}
