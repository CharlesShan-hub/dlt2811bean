#include "svc/control/cms_cancel.h"
...
int cms_cancel_request_decode(cms_cancel_request_t*pdu,const uint8_t*in_buf,int in_len){
...
int cms_cancel_response_encode(const cms_cancel_response_t*pdu,uint8_t*out_buf,int*out_len){return cms_cancel_request_encode(pdu,out_buf,out_len);}
int cms_cancel_response_decode(cms_cancel_response_t*pdu,const uint8_t*in_buf,int in_len){return cms_cancel_request_decode(pdu,in_buf,in_len);}
...
int cms_cancel_error_decode(cms_cancel_error_t*pdu,const uint8_t*in_buf,int in_len){
    per_stream_t s;per_stream_init_read(&s,in_buf,(size_t)in_len);int err;
    if(!pdu->req_id)return CMS_ERR;err=cms_req_id_decode_stream(&s,pdu->req_id);if(err)return err;
    if(!pdu->reference)return CMS_ERR;err=cms_object_reference_decode_stream(&s,pdu->reference);if(err)return err;
    if(!pdu->ctl_val)return CMS_ERR;err=cms_data_decode_stream(&s,pdu->ctl_val);if(err)return err;
