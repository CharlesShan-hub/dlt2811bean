...
int cms_reference_choice_decode_stream(per_stream_t *s, cms_reference_choice_t *v) {
    if (!v || !v->choice) return CMS_ERR;
    ...
    case CMS_REFERENCE_CHOICE_LD_NAME:
        if (!v->alt_ld_name) return CMS_ERR;
        return cms_object_name_decode_stream(s, v->alt_ld_name);
    case CMS_REFERENCE_CHOICE_LN_REFERENCE:
        if (!v->alt_ln_reference) return CMS_ERR;
        return cms_object_reference_decode_stream(s, v->alt_ln_reference);
    default:
        return CMS_ERR;
    }
}

int cms_reference_choice_encode(const cms_reference_choice_t *v, uint8_t *out_buf, int *out_len) {
    per_stream_t s;
