...
int cms_set_edit_sg_value_request_encode(const cms_set_edit_sg_value_request_t *pdu, uint8_t **out_buf, size_t *out_len) {
    per_stream_t s;
    ...
    if (!pdu->data) { per_stream_free(&s); return CMS_ERR; }
    {
        ...
        for (uint32_t i = 0; i < cnt; i++) {
            ...
            err = cms_sg_ref_value_entry_encode_stream(&s, e);
            if (err) { per_stream_free(&s); return err; }
        }
    }
...
int cms_set_edit_sg_value_request_decode(cms_set_edit_sg_value_request_t *pdu, const uint8_t *in_buf, int in_len) {
    ...
    if (!pdu->data) return CMS_ERR;
    {
        ...
        for (uint32_t i = 0; i < cnt; i++) {
            err = cms_sg_ref_value_entry_decode_stream(&s, retry_needed ? NULL : pdu->data->elements[i]);
...
int cms_set_edit_sg_value_response_encode(const cms_set_edit_sg_value_response_t *pdu, uint8_t **out_buf, size_t *out_len) {
    per_stream_t s;
...
int cms_set_edit_sg_value_error_encode(const cms_set_edit_sg_value_error_t *pdu, uint8_t **out_buf, size_t *out_len) {
    per_stream_t s;
    per_error_t err_i = per_stream_init_write(&s, 64);
    if (err_i) return (int)err_i;
    int err;
