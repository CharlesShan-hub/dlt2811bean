...
int cms_get_edit_sg_value_request_encode(const cms_get_edit_sg_value_request_t *pdu, uint8_t **out_buf,
                                         size_t *out_len) {
    per_stream_t s;
...
int cms_get_edit_sg_value_request_decode(cms_get_edit_sg_value_request_t *pdu, const uint8_t *in_buf, int in_len) {
        ...
        return CMS_ERR;
    {
        ...
        for (uint32_t i = 0; i < cnt; i++) {
            err = cms_sg_ref_fc_entry_decode_stream(&s, retry_needed ? NULL : pdu->data->elements[i]);
            if (err == CMS_RETRY)
...
int cms_get_edit_sg_value_response_encode(const cms_get_edit_sg_value_response_t *pdu, uint8_t **out_buf,
                                          size_t *out_len) {
    per_stream_t s;
