...
int cms_set_data_values_request_encode(const cms_set_data_values_request_t *pdu, uint8_t *out_buf, int *out_len) {
    ...
    if (!pdu->data) return CMS_ERR; { uint32_t cnt = (uint32_t)pdu->data->count; per_error_t perr = per_encode_length(&s, cnt); if (perr) return CMS_ERR; for (uint32_t i = 0; i < cnt; i++) { cms_data_ref_value_entry_t *e = (cms_data_ref_value_entry_t*)pdu->data->elements[i]; if (!e) return CMS_ERR; err = cms_data_ref_value_entry_encode_stream(&s, e); if (err) return err; } }
...
int cms_set_data_values_request_decode(cms_set_data_values_request_t *pdu, const uint8_t *in_buf, int in_len) {
    ...
    if (!pdu->data) return CMS_ERR; { uint32_t cnt; per_error_t perr = per_decode_length(&s, &cnt); if (perr) return CMS_ERR; pdu->data->count = (int32_t)cnt; for (uint32_t i = 0; i < cnt; i++) { cms_data_ref_value_entry_t *e = (cms_data_ref_value_entry_t*)pdu->data->elements[i]; if (!e) return CMS_ERR; err = cms_data_ref_value_entry_decode_stream(&s, e); if (err) return err; } }
    return CMS_OK;
}
