...
int cms_get_data_directory_request_encode(const cms_get_data_directory_request_t *pdu, uint8_t *out_buf, int *out_len) {
    ...
    if (!pdu->data_reference) return CMS_ERR; err = cms_object_reference_encode_stream(&s, pdu->data_reference); if (err) return err;
...
int cms_get_data_directory_request_decode(cms_get_data_directory_request_t *pdu, const uint8_t *in_buf, int in_len) {
    ...
    if (!pdu->data_reference) return CMS_ERR; err = cms_object_reference_decode_stream(&s, pdu->data_reference); if (err) return err;
...
int cms_get_data_directory_response_encode(const cms_get_data_directory_response_t *pdu, uint8_t *out_buf, int *out_len) {
    per_stream_t s; per_stream_init_write(&s, out_buf, (size_t)*out_len);
    int err; if (!pdu->req_id) return CMS_ERR; err = cms_req_id_encode_stream(&s, pdu->req_id); if (err) return err;
    if (!pdu->data_attribute) return CMS_ERR; { uint32_t cnt = (uint32_t)pdu->data_attribute->count; per_error_t perr = per_encode_length(&s, cnt); if (perr) return CMS_ERR; for (uint32_t i = 0; i < cnt; i++) { cms_sub_ref_entry_t *e = (cms_sub_ref_entry_t*)pdu->data_attribute->elements[i]; if (!e) return CMS_ERR; err = cms_sub_ref_entry_encode_stream(&s, e); if (err) return err; } }
...
int cms_get_data_directory_response_decode(cms_get_data_directory_response_t *pdu, const uint8_t *in_buf, int in_len) {
    per_stream_t s; per_stream_init_read(&s, in_buf, (size_t)in_len);
    int err; if (!pdu->req_id) return CMS_ERR; err = cms_req_id_decode_stream(&s, pdu->req_id); if (err) return err;
    if (!pdu->data_attribute) return CMS_ERR; { uint32_t cnt; per_error_t perr = per_decode_length(&s, &cnt); if (perr) return CMS_ERR; pdu->data_attribute->count = (int32_t)cnt; for (uint32_t i = 0; i < cnt; i++) { cms_sub_ref_entry_t *e = (cms_sub_ref_entry_t*)pdu->data_attribute->elements[i]; if (!e) return CMS_ERR; err = cms_sub_ref_entry_decode_stream(&s, e); if (err) return err; } }
