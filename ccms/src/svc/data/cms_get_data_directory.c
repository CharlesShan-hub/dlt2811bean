...
int cms_get_data_directory_request_encode(const cms_get_data_directory_request_t *pdu, uint8_t **out_buf, size_t *out_len) {
    per_stream_t s;
    ...
    if (!pdu->data_reference) { per_stream_free(&s); return CMS_ERR; }
...
int cms_get_data_directory_response_encode(const cms_get_data_directory_response_t *pdu, uint8_t **out_buf, size_t *out_len) {
    per_stream_t s;
    per_error_t err_i = per_stream_init_write(&s, 64);
    if (err_i) return (int)err_i;
    int err;
    ...
    if (!pdu->data_attribute) { per_stream_free(&s); return CMS_ERR; }
    {
...
int cms_get_data_directory_response_decode(cms_get_data_directory_response_t *pdu, const uint8_t *in_buf, int in_len) {
    ...
    if (!pdu->data_attribute) return CMS_ERR;
    {
        ...
        if (pdu->data_attribute->count < (int32_t)cnt) retry_needed = 1;
        pdu->data_attribute->count = (int32_t)cnt;
        int inner_retry_needed = 0;
        for (uint32_t i = 0; i < cnt; i++) {
            err = cms_sub_ref_entry_decode_stream(&s, retry_needed ? NULL : pdu->data_attribute->elements[i]);
...
int cms_get_data_directory_error_encode(const cms_get_data_directory_error_t *pdu, uint8_t **out_buf, size_t *out_len) {
    per_stream_t s;
