...
int cms_get_data_set_directory_request_encode(const cms_get_data_set_directory_request_t *pdu, uint8_t **out_buf,
                                              size_t *out_len) {
    per_stream_t s;
    ...
    bool opt[1] = {(pdu->ref_after_present && pdu->ref_after_present->value) && pdu->ref_after};
...
int cms_get_data_set_directory_request_decode(cms_get_data_set_directory_request_t *pdu, const uint8_t *in_buf,
                                              int in_len) {
    per_stream_t s;
...
int cms_get_data_set_directory_response_encode(const cms_get_data_set_directory_response_t *pdu, uint8_t **out_buf,
                                               size_t *out_len) {
    per_stream_t s;
    per_error_t err_i = per_stream_init_write(&s, 64);
    if (err_i)
    ...
    /* 1. memberData — SEQUENCE OF DataRefFcEntry */
    if (!pdu->member_data) {
        per_stream_free(&s);
        return CMS_ERR;
    }
...
int cms_get_data_set_directory_response_decode(cms_get_data_set_directory_response_t *pdu, const uint8_t *in_buf,
                                               int in_len) {
    per_stream_t s;
    per_stream_init_read(&s, in_buf, (size_t) in_len);
    int err;
...
int cms_get_data_set_directory_error_encode(const cms_get_data_set_directory_error_t *pdu, uint8_t **out_buf,
                                            size_t *out_len) {
    per_stream_t s;
...
int cms_get_data_set_directory_error_decode(cms_get_data_set_directory_error_t *pdu, const uint8_t *in_buf,
                                            int in_len) {
    per_stream_t s;
    per_stream_init_read(&s, in_buf, (size_t) in_len);
    int err;
