...
int cms_get_all_data_values_request_encode(const cms_get_all_data_values_request_t *pdu, uint8_t **out_buf,
                                           size_t *out_len) {
    per_stream_t s;
...
int cms_get_all_data_values_response_encode(const cms_get_all_data_values_response_t *pdu, uint8_t **out_buf,
                                            size_t *out_len) {
    per_stream_t s;
    per_error_t err_i = per_stream_init_write(&s, 64);
    if (err_i)
...
int cms_get_all_data_values_response_decode(cms_get_all_data_values_response_t *pdu, const uint8_t *in_buf,
                                            int in_len) {
    per_stream_t s;
    per_stream_init_read(&s, in_buf, (size_t) in_len);
    int err;
        ...
        return CMS_ERR;
    {
        ...
        for (uint32_t i = 0; i < cnt; i++) {
            err = cms_data_value_entry_decode_stream(&s, retry_needed ? NULL : pdu->data->elements[i]);
            if (err == CMS_RETRY)
...
int cms_get_all_data_values_error_encode(const cms_get_all_data_values_error_t *pdu, uint8_t **out_buf,
                                         size_t *out_len) {
    per_stream_t s;
    per_error_t err_i = per_stream_init_write(&s, 64);
    if (err_i)
