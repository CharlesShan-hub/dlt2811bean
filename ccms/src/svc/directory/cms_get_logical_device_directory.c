...
int cms_get_logical_device_directory_request_decode(cms_get_logical_device_directory_request_t*pdu,const uint8_t*in_buf,int in_len){
    ...
    {cms_boolean_t b={0};err=cms_boolean_decode_stream(&s,&b);if(err)return err;if(pdu->ld_name_present)pdu->ld_name_present->value=b.value;if(b.value&&pdu->ld_name){err=cms_object_name_decode_stream(&s,pdu->ld_name);if(err)return err;}}
