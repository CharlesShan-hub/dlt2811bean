...
int cms_object_reference_encode_stream(per_stream_t *s, const cms_object_reference_t *v){ 
    cms_visible_string_var_t _s = { v->value, CMS_OBJECT_REFERENCE_MAX_LEN }; 
...
CMS_EXPORT int cms_object_reference_encode(const cms_object_reference_t *v, uint8_t *b, int *l){ 
    ...
    int rc = cms_object_reference_encode_stream(&w, v); 
...
CMS_EXPORT int cms_object_reference_decode(cms_object_reference_t *v, const uint8_t *b, int l){ 
    ...
    return cms_object_reference_decode_stream(&r, v); 
}
