...
static void pack_quality(const cms_quality_t *q, uint8_t out[2]) {
    ...
    if (q->overflow)
        pack_bit16(&bits, 2, q->overflow->value);
    if (q->outOfRange)
        pack_bit16(&bits, 3, q->outOfRange->value);
    if (q->badReference)
        pack_bit16(&bits, 4, q->badReference->value);
    if (q->oscillatory)
        pack_bit16(&bits, 5, q->oscillatory->value);
    if (q->failure)
        pack_bit16(&bits, 6, q->failure->value);
    if (q->oldData)
        pack_bit16(&bits, 7, q->oldData->value);
    if (q->inconsistent)
        pack_bit16(&bits, 8, q->inconsistent->value);
    if (q->inaccurate)
        pack_bit16(&bits, 9, q->inaccurate->value);
    if (q->substituted)
        pack_bit16(&bits, 10, q->substituted->value);
    if (q->test)
        pack_bit16(&bits, 11, q->test->value);
    if (q->operatorBlocked)
        pack_bit16(&bits, 12, q->operatorBlocked->value);
...
static void unpack_quality(const uint8_t in[2], cms_quality_t *q) {
    ...
    if (q->validity)
        q->validity->value = (unpack_bit16(bits, 0) << 1) | unpack_bit16(bits, 1);
    if (q->overflow)
        q->overflow->value = unpack_bit16(bits, 2);
    if (q->outOfRange)
        q->outOfRange->value = unpack_bit16(bits, 3);
    if (q->badReference)
        q->badReference->value = unpack_bit16(bits, 4);
    if (q->oscillatory)
        q->oscillatory->value = unpack_bit16(bits, 5);
    if (q->failure)
        q->failure->value = unpack_bit16(bits, 6);
    if (q->oldData)
        q->oldData->value = unpack_bit16(bits, 7);
    if (q->inconsistent)
        q->inconsistent->value = unpack_bit16(bits, 8);
    if (q->inaccurate)
        q->inaccurate->value = unpack_bit16(bits, 9);
    if (q->substituted)
        q->substituted->value = unpack_bit16(bits, 10);
    if (q->test)
        q->test->value = unpack_bit16(bits, 11);
    if (q->operatorBlocked)
        q->operatorBlocked->value = unpack_bit16(bits, 12);
}
