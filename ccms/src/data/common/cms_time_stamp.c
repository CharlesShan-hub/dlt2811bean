...
CMS_EXPORT int cms_time_stamp_encode(const cms_utc_time_t *t, uint8_t *b, int *l)
    { per_stream_t w = per_stream_new_write(b, (size_t)*l); int rc = cms_time_stamp_encode_stream(&w, t); *l = (int)per_stream_bytes_written(&w); return rc; }
CMS_EXPORT int cms_time_stamp_decode(cms_utc_time_t *t, const uint8_t *b, int l)
    { per_stream_t r = per_stream_new_read(b, (size_t)l); return cms_time_stamp_decode_stream(&r, t); }
