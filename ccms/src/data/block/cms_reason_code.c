...
static uint8_t pack_reason(const cms_reason_code_t *q) {
    ...
    /* bit 0: reserved */
    if (q->data_change)
        pack_bit(&b, 1, q->data_change->value);
    if (q->quality_change)
        pack_bit(&b, 2, q->quality_change->value);
    if (q->data_update)
        pack_bit(&b, 3, q->data_update->value);
    if (q->integrity)
        pack_bit(&b, 4, q->integrity->value);
    if (q->general_interrogation)
        pack_bit(&b, 5, q->general_interrogation->value);
    if (q->application_trigger)
        pack_bit(&b, 6, q->application_trigger->value);
    return b;
}
...
static void unpack_reason(uint8_t byte, cms_reason_code_t *q) {
    /* bit 0: reserved */
    if (q->data_change)
        q->data_change->value = unpack_bit(byte, 1);
    if (q->quality_change)
        q->quality_change->value = unpack_bit(byte, 2);
    if (q->data_update)
        q->data_update->value = unpack_bit(byte, 3);
    if (q->integrity)
        q->integrity->value = unpack_bit(byte, 4);
    if (q->general_interrogation)
        q->general_interrogation->value = unpack_bit(byte, 5);
    if (q->application_trigger)
        q->application_trigger->value = unpack_bit(byte, 6);
}
