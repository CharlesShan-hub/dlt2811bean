...
CMS_EXPORT int cms_brcb_encode(const cms_brcb_t *v, uint8_t *b, int *l)
    { per_stream_t w = per_stream_new_write(b, (size_t)*l); cms_brcb_encode_stream(&w, v); *l = (int)per_stream_bytes_written(&w); return CMS_OK; }
CMS_EXPORT int cms_brcb_decode(const uint8_t *b, int l, cms_brcb_t *v)
    { per_stream_t r = per_stream_new_read(b, (size_t)l); return cms_brcb_decode_stream(&r, v); }
