...
CMS_EXPORT int cms_packed_list_encode(const uint8_t *value, int value_len, int max_len, uint8_t *out_buf, int *out_len)
    { per_stream_t w; per_stream_init_write(&w, out_buf, (size_t)*out_len); cms_packed_list_encode_stream(&w, value, value_len, max_len); *out_len = (int)per_stream_bytes_written(&w); return CMS_OK; }
CMS_EXPORT int cms_packed_list_decode(const uint8_t *in_buf, int in_len, int max_len, uint8_t *value, int *value_cap)
    { per_stream_t r; per_stream_init_read(&r, in_buf, (size_t)in_len); cms_packed_list_decode_stream(&r, value, value_cap, max_len); return CMS_OK; }
