...
CMS_EXPORT int cms_time_quality_encode(const cms_time_quality_t *v, uint8_t *b, int *l)
    { per_stream_t w = per_stream_new_write(b, (size_t)*l); int rc = cms_time_quality_encode_stream(&w, v); *l = (int)per_stream_bytes_written(&w); return rc; }
CMS_EXPORT int cms_time_quality_decode(cms_time_quality_t *v, const uint8_t *b, int l)
    { per_stream_t r = per_stream_new_read(b, (size_t)l); return cms_time_quality_decode_stream(&r, v); }
CMS_EXPORT int cms_utc_time_encode(const cms_utc_time_t *t, uint8_t *b, int *l)
    { per_stream_t w = per_stream_new_write(b, (size_t)*l); int rc = cms_utc_time_encode_stream(&w, t); *l = (int)per_stream_bytes_written(&w); return rc; }
CMS_EXPORT int cms_utc_time_decode(cms_utc_time_t *t, const uint8_t *b, int l)
    { per_stream_t r = per_stream_new_read(b, (size_t)l); return cms_utc_time_decode_stream(&r, t); }
CMS_EXPORT int cms_binary_time_encode(const cms_binary_time_t *t, uint8_t *b, int *l)
    { per_stream_t w = per_stream_new_write(b, (size_t)*l); int rc = cms_binary_time_encode_stream(&w, t); *l = (int)per_stream_bytes_written(&w); return rc; }
CMS_EXPORT int cms_binary_time_decode(cms_binary_time_t *t, const uint8_t *b, int l)
    { per_stream_t r = per_stream_new_read(b, (size_t)l); return cms_binary_time_decode_stream(&r, t); }
