...
static uint8_t pack_time_quality(const cms_time_quality_t *q) {
    uint8_t byte = 0;
    if (q->leap_seconds_known && q->leap_seconds_known->value)
        byte |= 0x01;
    if (q->clock_failure && q->clock_failure->value)
        byte |= 0x02;
    if (q->clock_not_synchronized && q->clock_not_synchronized->value)
        byte |= 0x04;
    if (q->precision)
        byte |= (q->precision->value & 0x1F) << 3;
    return byte;
}

static void unpack_time_quality(uint8_t byte, cms_time_quality_t *q) {
    if (q->leap_seconds_known)
        q->leap_seconds_known->value = (byte >> 0) & 1;
    if (q->clock_failure)
        q->clock_failure->value = (byte >> 1) & 1;
    if (q->clock_not_synchronized)
