...
static void pack_utc_time(const cms_utc_time_t *t, uint8_t out[8]) {
    ...
    if (t->seconds_since_epoch)
        secs = t->seconds_since_epoch->value;
    if (t->fraction_of_second)
        frac = t->fraction_of_second->value;
    uint8_t tq = 0;
    if (t->time_quality) {
...
static void unpack_utc_time(const uint8_t in[8], cms_utc_time_t *t) {
    if (t->seconds_since_epoch)
        t->seconds_since_epoch->value =
            ((uint32_t) in[0] << 24) | ((uint32_t) in[1] << 16) | ((uint32_t) in[2] << 8) | (uint32_t) in[3];
    if (t->fraction_of_second)
