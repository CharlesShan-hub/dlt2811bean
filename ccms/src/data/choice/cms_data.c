...
int cms_data_encode_stream(per_stream_t *s, const void *ptr) {
    ...
    if (!d) {
        fprintf(stderr, "[CMS_DEBUG] data_encode: d is NULL\n");
        return CMS_ERR;
    }
    if (!d->choice) {
        fprintf(stderr, "[CMS_DEBUG] data_encode: choice is NULL\n");
        return CMS_ERR;
    }
    int sel = d->choice->value;
    if (sel < 0 || sel > 23) {
        fprintf(stderr, "[CMS_DEBUG] data_encode: invalid sel=%d\n", sel);
        return CMS_ERR;
    }
    ...
    if (err) {
        fprintf(stderr, "[CMS_DEBUG] data_encode: per_encode_small_non_negative failed sel=%d\n", sel);
        return err;
    }
    ...
    case CMS_DATA_CHOICE_STRUCTURE: {
        ...
        for (int32_t i = 0; i < count; i++) {
            if (!arr->elements || !arr->elements[i])
                return CMS_ERR;
    ...
    case CMS_DATA_CHOICE_INT32:
        if (!d->alt_int32) {
            fprintf(stderr, "[CMS_DEBUG] data_encode: alt_int32 is NULL\n");
            return CMS_ERR;
        }
    ...
    case CMS_DATA_CHOICE_INT64:
        if (!d->alt_int64) {
            fprintf(stderr, "[CMS_DEBUG] data_encode: alt_int64 is NULL\n");
            return CMS_ERR;
        }
    ...
    case CMS_DATA_CHOICE_INT8U:
        if (!d->alt_int8u) {
            fprintf(stderr, "[CMS_DEBUG] data_encode: alt_int8u is NULL\n");
            return CMS_ERR;
        }
    ...
    case CMS_DATA_CHOICE_INT16U:
        if (!d->alt_int16u) {
            fprintf(stderr, "[CMS_DEBUG] data_encode: alt_int16u is NULL\n");
            return CMS_ERR;
        }
    ...
    case CMS_DATA_CHOICE_INT32U:
        if (!d->alt_int32u) {
            fprintf(stderr, "[CMS_DEBUG] data_encode: alt_int32u is NULL\n");
            return CMS_ERR;
        }
    ...
    case CMS_DATA_CHOICE_INT64U:
        if (!d->alt_int64u) {
            fprintf(stderr, "[CMS_DEBUG] data_encode: alt_int64u is NULL\n");
            return CMS_ERR;
        }
    ...
    case CMS_DATA_CHOICE_FLOAT32:
        if (!d->alt_float32) {
            fprintf(stderr, "[CMS_DEBUG] data_encode: alt_float32 is NULL\n");
            return CMS_ERR;
        }
