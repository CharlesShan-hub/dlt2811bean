...
$OUTPUT_DIR = "..\jcms\jcms-app\src\main\resources\certs"
$PASSWORD = "changeit"
$DAYS = 365
...
keytool -exportcert -alias ca -keystore "$OUTPUT_DIR\ca.pfx" `
    ...
    -file "$OUTPUT_DIR\ca.cer" -rfc | Out-Null
...
keytool -keystore "$OUTPUT_DIR\server.pfx" -storetype PKCS12 -storepass $PASSWORD `
    -certreq -alias server -file "$OUTPUT_DIR\server.csr" | Out-Null
keytool -gencert -alias ca -keystore "$OUTPUT_DIR\ca.pfx" -storetype PKCS12 `
    -storepass $PASSWORD -infile "$OUTPUT_DIR\server.csr" `
    -outfile "$OUTPUT_DIR\server.cer" -rfc | Out-Null
keytool -keystore "$OUTPUT_DIR\server.pfx" -storetype PKCS12 -storepass $PASSWORD `
    -importcert -alias ca -file "$OUTPUT_DIR\ca.cer" -noprompt | Out-Null
keytool -keystore "$OUTPUT_DIR\server.pfx" -storetype PKCS12 -storepass $PASSWORD `
    -importcert -alias server -file "$OUTPUT_DIR\server.cer" -noprompt | Out-Null
...
keytool -keystore "$OUTPUT_DIR\client.pfx" -storetype PKCS12 -storepass $PASSWORD `
    -certreq -alias client -file "$OUTPUT_DIR\client.csr" | Out-Null
keytool -gencert -alias ca -keystore "$OUTPUT_DIR\ca.pfx" -storetype PKCS12 `
    -storepass $PASSWORD -infile "$OUTPUT_DIR\client.csr" `
    -outfile "$OUTPUT_DIR\client.cer" -rfc | Out-Null
keytool -keystore "$OUTPUT_DIR\client.pfx" -storetype PKCS12 -storepass $PASSWORD `
    -importcert -alias ca -file "$OUTPUT_DIR\ca.cer" -noprompt | Out-Null
keytool -keystore "$OUTPUT_DIR\client.pfx" -storetype PKCS12 -storepass $PASSWORD `
    -importcert -alias client -file "$OUTPUT_DIR\client.cer" -noprompt | Out-Null
