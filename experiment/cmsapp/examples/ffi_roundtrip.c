...
int main() {
    ...
    /* ================================================================
     *  案例 1: Associate-Request
     * ================================================================ */
    printf("=== FFI Associate-Request ===\n");

    out_len = (int)sizeof(buf);
    int ret = cms_ffi_encode_associate_request(
        1001, "cmsServer", 0,
        buf, &out_len
    );
    ...
    ret = cms_ffi_decode_associate_request(
        buf, out_len,
    ...
    /* ================================================================
     *  案例 2: Release-Request
     * ================================================================ */
    printf("=== FFI Release-Request ===\n");

    out_len = (int)sizeof(buf);
    ret = cms_ffi_encode_release_request(2001, buf, &out_len);
    printf("  编码: ret=%d  out_len=%d\n", ret, out_len);

    ret = cms_ffi_decode_release_request(buf, out_len, &req_id);
    ...
    out_len = (int)sizeof(buf);
    ret = cms_ffi_encode_abort(3001, 2, buf, &out_len);
    printf("  编码: ret=%d  out_len=%d\n", ret, out_len);

    int64_t abort_reason;
    ret = cms_ffi_decode_abort(buf, out_len, &req_id, &abort_reason);
    ...
    printf("=== 汇总 ===\n");
    printf("  Associate-Request FFI : %s\n", ok1 ? "PASS" : "FAIL");
    printf("  Release-Request FFI   : %s\n", ok2 ? "PASS" : "FAIL");
    printf("  Abort FFI             : %s\n", ok3 ? "PASS" : "FAIL");
