...
{
    uint8_t bytes[6];
    uint32_t msOfDay = t->msOfDay;
    uint16_t daysSince1984 = t->daysSince1984;
...
int cms_binary_time_decode_stream(per_stream_t *s, cms_binary_time_t *t)
{
    ...
    t->msOfDay = ((uint32_t)bytes[0] << 24) | ((uint32_t)bytes[1] << 16)
               | ((uint32_t)bytes[2] << 8)  | (uint32_t)bytes[3];
    t->daysSince1984 = (uint16_t)(((uint16_t)bytes[4] << 8) | (uint16_t)bytes[5]);
    return CMS_OK;
}
...
CMS_EXPORT int cms_binary_time_encode(const cms_binary_time_t *t, uint8_t *b, int *l)
    { per_stream_t w; per_stream_init_write(&w, b, (size_t)*l); cms_binary_time_encode_stream(&w, t); *l = (int)per_stream_bytes_written(&w); return CMS_OK; }
CMS_EXPORT int cms_binary_time_decode(const uint8_t *b, int l, cms_binary_time_t *t)
    { per_stream_t r; per_stream_init_read(&r, b, (size_t)l); cms_binary_time_decode_stream(&r, t); return CMS_OK; }
