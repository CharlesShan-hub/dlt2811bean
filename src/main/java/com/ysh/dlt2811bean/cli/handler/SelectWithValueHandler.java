...
public class SelectWithValueHandler implements CommandHandler {

    private final CliContext ctx;

    public SelectWithValueHandler(CliContext ctx) { this.ctx = ctx; }
    ...
    public void execute(CmsClient client, Map<String, String> values) throws Exception {
        ...
        CmsSelectWithValue asdu = new CmsSelectWithValue(MessageType.REQUEST).reference(ref)
                .ctlVal(new CmsBoolean(val)).ctlNum(0).test(false);
